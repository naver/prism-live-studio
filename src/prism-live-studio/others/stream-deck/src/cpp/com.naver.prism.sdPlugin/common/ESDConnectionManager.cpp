...
void ESDConnectionManager::OnClose(WebsocketClient *inClient, websocketpp::connection_hdl inConnectionHandler)
{
	...
	if (mPlugin) {
		mPlugin->StreamDeckDisconnected();
	}
...
ESDConnectionManager::ESDConnectionManager(int inPort, const std::string &inPluginUUID, const std::string &inRegisterEvent, const std::string &inInfo, PLSPluginBase *inPlugin)
	:
...
void ESDConnectionManager::Run(websocketpp::lib::asio::io_service *ios)
{
	assert(ios);
	try {
		// Create the endpoint
		...
		mWebsocket.init_asio(ios, error);
...
void ESDConnectionManager::SetState(StateType inState, const std::string &inContext)
{
	json jsonObject;
