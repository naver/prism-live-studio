...
{
	username_pw_set("client", MQTT_SERVER_PW.toUtf8().constData());
	threaded_set();
}
...
void PLSMosquitto::start(int iVideoSeq)
{
	...
	m_strVideoSequence = VideoSequence;
	...
	m_thread = QThread::create([this] {
		...
		auto retCode = mosqpp::mosquittopp::connect(m_mqttUrl.toUtf8().constData(), 80, 10);
		pls_check_app_exiting();
		if (retCode != MOSQ_ERR_SUCCESS) {
			PLS_LIVE_INFO(MODULE_PlatformService, "MQTT status: start connected mqtt error");
		}

		PLS_LIVE_INFO(MODULE_MQTT, "MQTT status: mqtt start child thread loop.start,Thread=%d connect retCode: %d,mqttUrl=%s", this_thread::get_id(), retCode, m_mqttUrl.toUtf8().constData());
		m_connected.release();
		loop_forever();
		pls_check_app_exiting();
		PLS_LIVE_INFO_EX(MODULE_MQTT, m_strVideoSequence.c_str(), "MQTT status: mqtt start child thread loop.end,Thread=%d", this_thread::get_id());
	});
	m_thread->start();
}
...
void PLSMosquitto::stop()
{
	...
	std::thread([this] {
		...
		deleteLater();
		pls_check_app_exiting();
		PLS_LIVE_INFO_EX(MODULE_MQTT, m_strVideoSequence.c_str(), "MQTT status: mqtt end stop,Thread=%d", this_thread::get_id());
	}).detach();
}

void PLSMosquitto::subscribleAll()
{
	PLS_LIVE_INFO(MODULE_MQTT, "MQTT status: mqtt subscribe all ,Thread=%d,mqttUrl=%s", this_thread::get_id(), m_mqttUrl.toUtf8().constData());
	auto stat = QString("prism/live/%1/pub/stat").arg(m_iVideoSeq);
	subscribe(nullptr, stat.toUtf8().constData());

	auto status = QString("prism/live/%1/priv/status").arg(m_iVideoSeq);
	subscribe(nullptr, status.toUtf8().constData());

	auto chat = QString("prism/live/%1/pub/chat").arg(m_iVideoSeq);
	subscribe(nullptr, chat.toUtf8().constData());
}

void PLSMosquitto::on_connect(int status)
{
	PLS_LIVE_INFO(MODULE_MQTT, "MQTT status: mqtt on_connect,Thread=%d,mqttUrl=%s,status=%d", this_thread::get_id(), m_mqttUrl.toUtf8().constData(), status);
	subscribleAll();
}

void PLSMosquitto::on_disconnect(int status)
{
	PLS_LIVE_INFO_EX(MODULE_MQTT, m_strVideoSequence.c_str(), "MQTT status: mqtt on_disconnect,Thread=%d,mqttUrl=%s,status=%d", this_thread::get_id(), m_mqttUrl.toUtf8().constData(), status);
}
