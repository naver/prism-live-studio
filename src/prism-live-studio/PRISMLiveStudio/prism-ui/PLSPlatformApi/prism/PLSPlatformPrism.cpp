...
PLSPlatformPrism::PLSPlatformPrism() : m_timerHeartbeat(this)
{
	...
	connect(&m_timerHeartbeat, &QTimer::timeout, this, [this]() {
		...
		if (auto iVideoSeq = m_iVideoSeq[DualOutputType::Horizontal]; 0 != iVideoSeq && !PLS_PLATFORM_API->isPrismLive(DualOutputType::Horizontal)) {
			requests.add(requestHeartbeat(iVideoSeq));
		}
		if (auto iVideoSeq = m_iVideoSeq[DualOutputType::Vertical]; 0 != iVideoSeq && !PLS_PLATFORM_API->isPrismLive(DualOutputType::Vertical)) {
			requests.add(requestHeartbeat(iVideoSeq));
		}
...
{
	//"/prism/output/quality"
	return pls::http::Request()
		.method(pls::http::Method::Post)
		.hmacUrl(PRISM_API_STATUS.arg(PRISM_HOST_REAL).arg(apiPath), PRISM_PC_HMAC_KEY_REAL)
		.jsonContentType()
...
void PLSPlatformPrism::onInactive(PLSPlatformBase *platform, bool value)
{
	...
	if (bDualOutput) {
		...
		auto outputType = bVerticalOutput ? DualOutputType::Vertical : DualOutputType::Horizontal;
		auto platforms = bVerticalOutput ? PLS_PLATFORM_API->getVerticalPlatforms() : PLS_PLATFORM_API->getHorizontalPlatforms();

		if (platforms.empty()) {
			PLSBasic::instance()->StopStreaming(outputType);
...
void PLSPlatformPrism::onPrepareLive(bool value)
{
	...
	// Check if this is a resume streaming (crash recovery)
	if (PLS_PLATFORM_API->getResumeStreamingFlag()) {
		resumeStreaming();
		return;
	}

	bool bContainDirect = false;
	pls::http::Requests requests;
	if (pls_is_dual_output_on()) {
		auto platformListH = PLS_PLATFORM_HORIZONTAL;
		auto platformListV = PLS_PLATFORM_VERTICAL;

		bContainDirect = (1 == platformListH.size()) || (1 == platformListV.size());
		requests.add(requestStartSimulcastLive(platformListH.size() > 1, platformListH, DualOutputType::Horizontal));
		requests.add(requestStartSimulcastLive(platformListV.size() > 1, platformListV, DualOutputType::Vertical));
	} else {
		auto platformList = PLS_PLATFORM_ACTIVIED;

		bContainDirect = 1 == platformList.size();
		requests.add(requestStartSimulcastLive(platformList.size() > 1, platformList, DualOutputType::Horizontal));
	}

	pls::http::requests(requests //
				    .receiver(pls_get_main_view())
				    .results([this, bContainDirect](const pls::http::Replies &replies) {
					    pls_async_call(this, [this, bContainDirect] {
						    ...
						    // Save streaming state for crash recovery after Prism API success
						    if (bSuccessful) {
							    PLS_PLATFORM_API->saveStreamingState();
						    }

						    prepareLiveCallback(bSuccessful);

						    if (bSuccessful && bContainDirect) {
...
void PLSPlatformPrism::resumeStreaming()
{
	PLS_INFO(MODULE_PlatformService, "Resume streaming mode: skip Prism API call, use saved state");

	// Restore videoSeq from saved state
	const QJsonObject &state = PLS_PLATFORM_API->getResumeStreamingState();
	QJsonObject prismObj = state[QStringLiteral("prism")].toObject();

	int videoSeqH = prismObj[QStringLiteral("videoSeq_horizontal")].toInt();
	m_iVideoSeq[DualOutputType::Horizontal] = videoSeqH;
	if (pls_is_dual_output_on() && PLS_PLATFORM_API->getVerticalPlatforms().size() > 0) {
		int videoSeqV = prismObj[QStringLiteral("videoSeq_vertical")].toInt();
		m_iVideoSeq[DualOutputType::Vertical] = videoSeqV;
	}

	// Parse streaming mode using QMetaEnum
	using StreamingMode = PLSPlatformApi::StreamingMode;
	QString streamingModeStr = state[QStringLiteral("streamingMode")].toString();
	QMetaEnum metaEnum = QMetaEnum::fromType<StreamingMode>();
	int modeValue = metaEnum.keyToValue(streamingModeStr.toUtf8().constData());
	auto streamingMode = static_cast<StreamingMode>(modeValue);
	// Restore stream server/key based on streaming mode
	QString serverH, keyH, serverV, keyV;
	switch (streamingMode) {
	case StreamingMode::DualHMultiVMulti:
		serverH = prismObj[QStringLiteral("streamServer_horizontal")].toString();
		keyH = prismObj[QStringLiteral("streamKey_horizontal")].toString();
		serverV = prismObj[QStringLiteral("streamServer_vertical")].toString();
		keyV = prismObj[QStringLiteral("streamKey_vertical")].toString();
		m_strPublishUrl[DualOutputType::Horizontal] = QStringLiteral("%1/%2").arg(serverH, keyH).toStdString();
		m_strPublishUrl[DualOutputType::Vertical] = QStringLiteral("%1/%2").arg(serverV, keyV).toStdString();
		PLS_PLATFORM_API->saveStreamSettings("Prism", serverH.toStdString(), keyH.toStdString(), false);
		PLS_PLATFORM_API->saveStreamSettings("Prism", serverV.toStdString(), keyV.toStdString(), true);
		break;
	...
	case StreamingMode::DualHSingleVMulti:
		...
		break;
	default:
		// Single platform or non-multi mode: stream settings handled by platform
		break;
	}

	PLS_INFO(MODULE_PlatformService, "Resume streaming: videoSeq_H=%d, mode=%s", videoSeqH, streamingModeStr.toUtf8().constData());

	// Skip Prism API, directly call prepare callback
	prepareLiveCallback(true);

	if (streamingMode == StreamingMode::DualHSingleVSingle || streamingMode == StreamingMode::Single || streamingMode == StreamingMode::DualHSingleVMulti ||
	    streamingMode == StreamingMode::DualHMultiVSingle) {
		m_timerHeartbeat.start();
	}
}
...
{
	// Clear streaming state on normal live ended
	PLS_PLATFORM_API->clearStreamingState();
	...
	if (0 != m_iVideoSeq[DualOutputType::Horizontal]) {
		requests.add(requestStopSimulcastLive(PLS_PLATFORM_API->isPrismLive(DualOutputType::Horizontal), m_iVideoSeq[DualOutputType::Horizontal]));
		m_iVideoSeq[DualOutputType::Horizontal] = 0;
	}
	if (0 != m_iVideoSeq[DualOutputType::Vertical]) {
		requests.add(requestStopSimulcastLive(PLS_PLATFORM_API->isPrismLive(DualOutputType::Vertical), m_iVideoSeq[DualOutputType::Vertical]));
...
pls::http::Request PLSPlatformPrism::requestStartSimulcastLive(bool bPrism, list<PLSPlatformBase *> platformList, DualOutputType outputType)
{
	...
	for (auto item : platformList) {
		...
		auto url = item->getStreamServer();
		auto key = item->getStreamKey();
		if (auto posUrl = url.rfind('?'); string::npos != posUrl) {
			if (auto posKey = key.rfind('?'); string::npos != posKey) {
				key.push_back('&');
				key.append(url.substr(posUrl + 1));
			} else {
				key.append(url.substr(posUrl));
			}
			url.erase(posUrl);
		}

		jsonObjecPlatform["streamUrl"] = QString::fromStdString(url);
		jsonObjecPlatform["streamKey"] = QString::fromStdString(key);
		...
		if (!bPrism) {
			...
			jsonObjectRoot["publishUrl"] = directRtmlUrl.toUtf8().constData();
		}
	...
	auto request = pls::http::Request() //
			       .method(pls::http::Method::Post)
			       .timeout(PLS_SYNC_SERVER_MANAGE->getPrismLiveStartRequestTimeoutMs())
			       ...
			       .okResult([this, bPrism, url, platformList, outputType](const pls::http::Reply &reply) {
				       ...
				       auto urlEn = reply.request().originalUrl().path();
				       pls_async_call(this, [this, doc, bPrism, statusCode, url, urlEn, platformList, outputType] {
					       if (!doc.isObject()) {
						       ...
						       if (pls_is_dual_output_on()) {
							       m_listApiError.emplace_back(platformList, PLSErrorHandler::getAlertStringByCustomErrName(customErrorFailedToStartLive(), getTableName(),
																			PLSErrorHandler::ExtraData(urlEn)));
						       } else {
							       PLSErrorHandler::showAlertByCustomErrName(customErrorFailedToStartLive(), getTableName(), PLSErrorHandler::ExtraData(urlEn));
						       }
					       } else {
			       ...
			       .failResult([this, url, bPrism, platformType, platformList, outputType](const pls::http::Reply &reply) {
				       ...
				       pls_async_call(this, [this, bPrism, statusCode, url, platformList, isTimeout, urlEn, data, platformType, error] {
					       // get live start api fail reason
					       LiveAbortStage stage = bPrism ? LiveAbortStage::LiveStartRequestFailed : LiveAbortStage::LiveDirectStartRequestFailed;
					       QString reason = PLS_PLATFORM_API->getLiveAbortReason(stage);
					       ...
					       if (isTimeout) {
						       ...
						       if (pls_is_dual_output_on()) {
							       m_listApiError.emplace_back(platformList,
											   PLSErrorHandler::getAlertStringByCustomErrName(customErrorTimeoutTryAgain(), getTableName(), extraData));
						       } else {
							       PLSErrorHandler::showAlertByCustomErrName(customErrorTimeoutTryAgain(), getTableName(), extraData);
						       }
						       return;
					       }
					       ...
					       if (platformType == PrismErrorPlatformType::NCPPlatform) {
						       auto retData = PLSErrorHandler::getAlertString({statusCode, error, data}, NCB2B, QString(), exData);
						       if (retData.isMatched) {
							       if (pls_is_dual_output_on()) {
								       m_listApiError.emplace_back(platformList, retData);
							       } else {
								       PLSErrorHandler::directShowAlert(retData);
							       }
							       return;
					       ...
					       exData.isShowUnknownError = true;
					       auto retData = PLSErrorHandler::getAlertString({statusCode, error, data}, getTableName(), customErrorServerErrorTryAgain(), exData);
					       if (PLSErrorHandler::PRISM_API_TOKEN_EXPIRED == retData.prismCode) {
						       QMetaObject::invokeMethod(this, [this, retData] { onTokenExpired(retData); }, Qt::QueuedConnection);
						       return;
					       } else {
						       if (pls_is_dual_output_on()) {
							       m_listApiError.emplace_back(platformList, retData);
	...
	PLS_LIVE_INFO(MODULE_PlatformService, "start living url:%s", url.toUtf8().constData());
							...
							list<PLSPlatformBase *> platformList)
{
	...
	if (errorCode == 0) {
		...
		PLS_LIVE_INFO(MODULE_PlatformService, "request url:%s success", url.toUtf8().constData());
		PLS_INFO_KR(MODULE_PlatformService, "request url:%s success, videoSeq is %d  ", url.toUtf8().constData(), m_iVideoSeq);
		if (bPrism) {
		...
		if (pls_is_dual_output_on()) {
			m_listApiError.push_back(
				{platformList, PLSErrorHandler::getAlertStringByCustomErrName(customErrorFailedToStartLive(), getTableName(), PLSErrorHandler::ExtraData("StartSimulcastLive"))});
		} else {
			pls_async_call_mt(this, [] {
				PLSErrorHandler::showAlertByCustomErrName(customErrorFailedToStartLive(), getTableName(), PLSErrorHandler::ExtraData("StartSimulcastLive"));
			});
		}
	}
}
...
{
	PLS_LIVE_ERROR(MODULE_PlatformService, "request url:%s error: %d-%s", url.toUtf8().constData(), code, QString(data).toUtf8().constData());
}
...
pls::http::Request PLSPlatformPrism::requestStopSimulcastLive(bool bPrism, int iVideoSeq)
{
	...
	PLS_LIVE_INFO(MODULE_PlatformService, "stop living url:%s", url.toUtf8().constData());
...
{
	auto url = urlForPath(
		QString("/live/%1/simulcast").arg((pls_is_dual_output_on() && platform->getVerticalOutput()) ? m_iVideoSeq[DualOutputType::Vertical] : m_iVideoSeq[DualOutputType::Horizontal]));
	...
	PLS_LIVE_INFO(MODULE_PlatformService, "stop single live request url:%s", url.toUtf8().constData());
}
...
{
	auto curStatus = platform->getTokenRequestStatus();
	if (curStatus == PLSTokenRequestStatus::PLS_ING || curStatus == PLSTokenRequestStatus::PLS_BAD) {
		const char *platName = platform->getNameForChannelType();
		PLS_INFO(MODULE_PlatformService, "ignore refresh %s token, because status is: %s", platName, PLSTokenRequestStatus::PLS_BAD == curStatus ? "bad" : "ing");
		if (callback) {
			callback(false);
		}
		return;
	...
	if (platform->getServiceType() == PLSServiceType::ST_YOUTUBE || platform->getServiceType() == PLSServiceType::ST_NCB2B) {
		...
		requestRefreshAccessToken(platform, [platform, uuid, callback](bool isSucceed) {
			...
			if (!isSucceed && PLSCHANNELS_API->getChannelStatus(uuid) != ChannelData::Expired) {
	...
	} else if (platform->getServiceType() == PLSServiceType::ST_TWITCH) {
		platform->setTokenRequestStatus(PLSTokenRequestStatus::PLS_ING);
		requestTwitchRefreshToken(callback, platform, uuid, true);
	} else {
...
void PLSPlatformPrism::twitchRefreshTokenFinished(bool isRefresh, const PLSPlatformBase *platform, const QString &uuid, const std::function<void(bool)> &callback) const
{
	bool _isSucceed = false;
	if (!isRefresh) {
		pls_toast_message(pls_toast_info_type::PLS_TOAST_NOTICE, QTStr("Live.Check.LiveInfo.Refresh.Expired").arg(platform->getChannelName()));
		PLSCHANNELS_API->setChannelStatus(uuid, ChannelData::Expired);
		PLS_INFO(MODULE_PlatformService, "refresh twitch token expired");
	} else {
		PLS_INFO(MODULE_PlatformService, "refresh twitch token success");
		QJsonObject jsonObjectRoot;
		jsonObjectRoot["simulcastSeq"] = platform->getChannelLiveSeq();
		jsonObjectRoot["livePlatform"] = platform->getNameForLiveStart();
...
void PLSPlatformPrism::requestTwitchRefreshToken(const std::function<void(bool)> &callback, PLSPlatformBase *platform, const QString &uuid, bool isForceUpdate) const
{
	auto callbackFun = [platform, uuid, callback, this](bool isRefresh) {
		platform->setTokenRequestStatus(isRefresh ? PLSTokenRequestStatus::PLS_GOOD : PLSTokenRequestStatus::PLS_BAD);
		this->twitchRefreshTokenFinished(isRefresh, platform, uuid, callback);
	};
	auto twitchPlatform = static_cast<PLSPlatformTwitch *>(platform);
	if (twitchPlatform) {
		PLS_INFO(MODULE_PlatformService, "start refresh twitch token request");
		twitchPlatform->pollingCheckToken(isForceUpdate, callbackFun);
	}
}
...
#define BOOL_To_STR(x) (x) ? "true" : "false"
	...
	PLS_LIVE_INFO(MODULE_PlatformService, "%s", log.toUtf8().constData());
	...
	PLS_LIVE_INFO_KR(MODULE_PlatformService, "%s", log_kr.toUtf8().constData());
}
