...
- (void)drawInContext:(CGContextRef)context size:(CGSize)size
{
	...
	for (int i = 0; i < length; i++) {
		...
		CGPathAddCurveToPoint(self.cgPath, nullptr, self.ctrlPoints->at(s + 1).x, self.ctrlPoints->at(s + 1).y, self.ctrlPoints->at(s + 2).x, self.ctrlPoints->at(s + 2).y,
				      self.ctrlPoints->at(s + 3).x, self.ctrlPoints->at(s + 3).y);
	}
...
- (id)initWithStrokeColor:(NSColor *)strokeColor
	    ...
	    useCtrlPoints:(BOOL)useCtrlPoints
		 strokeID:(NSInteger)strokeID
{
	self = [super initWithStrokeColor:strokeColor brushMode:Glow colorMode:colorMode strokeWidth:strokeWidth thicknessMode:thicknessMode useCtrlPoints:useCtrlPoints strokeID:(NSInteger)strokeID];
...
- (void)drawInContext:(CGContextRef)context size:(CGSize)size
{
	...
	for (int i = 0; i < self.ctrlPoints->size() / 4; i++) {
		...
		CGPathAddCurveToPoint(self.cgSubPath, nullptr, self.ctrlPoints->at(s + 1).x, self.ctrlPoints->at(s + 1).y, self.ctrlPoints->at(s + 2).x, self.ctrlPoints->at(s + 2).y,
				      self.ctrlPoints->at(s + 3).x, self.ctrlPoints->at(s + 3).y);
	}
...
- (void)drawInContext:(CGContextRef)context size:(CGSize)size
{
	...
	self.cgPath = [self getPathFromPoint:NSMakePoint(self.points->front().x, self.points->front().y) toPoint:NSMakePoint(self.points->back().x, self.points->back().y) width:2];
	CGContextAddPath(context, self.cgPath);
	CGContextDrawPath(context, kCGPathFillStroke);
	CGContextRestoreGState(context);
}

- (CGMutablePathRef)getPathFromPoint:(CGPoint)startPoint toPoint:(CGPoint)endPoint width:(CGFloat)width
{
...
- (void)getRectPoints:(CGPoint[4])points forLength:(CGFloat)length width:(CGFloat)width
{
...
- (id)initWithStrokeColor:(NSColor *)strokeColor
		...
		headWidth:(CGFloat)headWidth
		 ...
		 strokeID:(NSInteger)strokeID
{
	self = [super initWithStrokeColor:strokeColor brushMode:ArrowLine colorMode:colorMode strokeWidth:strokeWidth thicknessMode:thicknessMode useCtrlPoints:true strokeID:(NSInteger)strokeID];
...
- (CGMutablePathRef)dqd_bezierPathWithArrowFromPoint:(CGPoint)startPoint toPoint:(CGPoint)endPoint tailWidth:(CGFloat)tailWidth headWidth:(CGFloat)headWidth headLength:(CGFloat)headLength
{
	CGFloat length = hypotf(endPoint.x - startPoint.x, endPoint.y - startPoint.y);
