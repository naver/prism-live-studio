...
#ifndef MAC_DEMO
extern bool uploadToGPU(gs_texture_t *texture, const uint8_t *data, uint32_t width, uint32_t height, uint32_t stride, bool flip);
#endif
...
void PLSStrokeManagerImpl::beginDraw(unsigned int brushMode, unsigned int colorMode, unsigned int thicknessMode, PointF point)
{
	NSPoint p;
	p.x = point.x;
	p.y = point.y;
	[(__bridge id)self beginWithBushMode:(BrushMode)brushMode startPoint:p colorMode:(ColorMode)colorMode thicknessMode:(ThicknessMode)thicknessMode];
}
...
void PLSStrokeManagerImpl::setCallback(void *context, DrawPenCallBacak cb)
{
	...
	void (^myBlock)(void *context, bool undoEmpty, bool redoEmpty) = ^(void *context, bool undoEmpty, bool redoEmpty) {
		cb(context, undoEmpty, redoEmpty);
	};
...
- (void)_beginWithBushMode:(BrushMode)mode startPoint:(NSPoint)point colorMode:(ColorMode)colorMode thicknessMode:(ThicknessMode)thicknessMode strokeID:(NSInteger)strokeID
{
...
- (void)beginWithBushMode:(BrushMode)mode startPoint:(NSPoint)p colorMode:(ColorMode)colorMode thicknessMode:(ThicknessMode)thicknessMode
{
	__weak PLSStrokeManager *weakSelf = self;
	dispatch_async(_renderQueue, ^{
		if (weakSelf.currentStroke) {
			return;
		}
		...
		[weakSelf _beginWithBushMode:mode startPoint:point colorMode:colorMode thicknessMode:thicknessMode strokeID:-1];
		[weakSelf _drawPensWithRedraw:FALSE rightNow:TRUE yOffset:0.0 andDebugInfo:@"beginWithBushMode"];
	});
}
...
- (void)moveTo:(NSPoint)p
{
	__weak PLSStrokeManager *weakSelf = self;
	dispatch_async(_renderQueue, ^{
		if (weakSelf.currentStroke == NULL) {
			return;
		}
		...
		if (weakSelf.currentStroke.points->size() > MAX_STROKE_POINT_COUNT) {
			...
			dispatch_async(weakSelf.renderQueue, ^{
				[weakSelf _beginWithBushMode:brushMode startPoint:point colorMode:colorMode thicknessMode:thicknessMode strokeID:strokeID];
			});
		} else if ([weakSelf.currentStroke moveToPoint:point]) {
			[weakSelf _drawPensWithRedraw:FALSE rightNow:NO yOffset:0.0 andDebugInfo:@"moveTo"];
		}
	});
}
...
- (void)_endTo:(NSPoint)point
{
	...
	[self _drawPensWithRedraw:NO rightNow:TRUE yOffset:0.0 andDebugInfo:@"endTo"];
}
...
- (void)_eraseOn:(NSPoint)point
{
	...
	if (erasedStrokeID >= 0) {
		...
		[self _drawPensWithRedraw:TRUE rightNow:TRUE yOffset:0.0 andDebugInfo:@"_eraseOn"];
	}
...
- (void)_undo
{
	...
	case EraseAction: {
		...
		if (bRefresh) {
			[self _drawPensWithRedraw:TRUE rightNow:TRUE yOffset:0.0 andDebugInfo:@"_undo 1"];
		}
	} break;
	case DrawAction: {
		...
		if (strokeID >= 0) {
			...
			[self _drawPensWithRedraw:TRUE rightNow:TRUE yOffset:0.0 andDebugInfo:@"_undo 2"];
		}
	} break;
...
- (void)_redo
{
	...
	case EraseAction: {
		...
		if (bRefresh) {
			[self _drawPensWithRedraw:TRUE rightNow:TRUE yOffset:0.0 andDebugInfo:@"_redo 1"];
		}
	} break;
	...
	case DrawAction: {
		...
		if (strokeID >= 0) {
			...
			[self _drawPensWithRedraw:TRUE rightNow:TRUE yOffset:0.0 andDebugInfo:@"_redo 2"];
		}
...
- (void)_clear
{
	...
	[self _drawPensWithRedraw:TRUE rightNow:TRUE yOffset:0.0 andDebugInfo:@"_clear"];
}
...
- (void)resize:(NSSize)size
{
	...
	dispatch_async(self.renderQueue, ^{
		...
		[weakSelf _drawPensWithRedraw:TRUE rightNow:TRUE yOffset:yOffset andDebugInfo:@"resize"];
	});
}
...
- (void)_drawPensWithRedraw:(BOOL)needsRedraw rightNow:(BOOL)rightNow yOffset:(CGFloat)yOffset andDebugInfo:(NSString *)info
{
	if (!self.isVisible) {
#if MAC_DEMO
		dispatch_async(dispatch_get_main_queue(), ^{
	...
	if (needsRedraw || rightNow || self.lastDrawTime == nullptr || timeIntervalSinceNow >= 50) { // limit draw frequency
		...
		CGContextRef imageContext = CGBitmapContextCreate(self.currentCanvasRawData, iW, iH, 8, iW * 4, colorSpace,
								  kCGBitmapByteOrder32Big | kCGImageAlphaPremultipliedLast); // `kCGBitmapByteOrder32Big | kCGImageAlphaPremultipliedLast` == `GS_RGBA`
		if (needsRedraw) {
			for (int i = 0; i < self.drawingActions.count; i++) {
				PLSStrokeBase *pen = (PLSStrokeBase *)self.drawingActions[i];
...
- (void)drawPensWithRedraw:(BOOL)needsRedraw rightNow:(BOOL)rightNow andDebugInfo:(NSString *)info
{
	__weak PLSStrokeManager *weakSelf = self;
	dispatch_async(self.renderQueue, ^{
		[weakSelf _drawPensWithRedraw:needsRedraw rightNow:rightNow yOffset:0.0 andDebugInfo:info];
	});
}
...
#ifndef MAC_DEMO
bool uploadToGPU(gs_texture_t *texture, const uint8_t *data, uint32_t width, uint32_t height, uint32_t stride, bool flip)
{
