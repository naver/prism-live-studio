...
private:
	static void callback__(DNSServiceRef sdRef,
...
void DNSServiceResolver::Resolve(std::function<void (uint16_t)> closure)
{
	...
	name = (name != NULL) ? [name stringByAppendingString:@".prism-vcam-info"] : @"prism-vcam-info";
	NSString *type = @"_prismvcam._tcp";
	NSString *domain = @"local";
	
	DNSServiceErrorType dnsError = DNSServiceResolve(&sd_ref_,
													 0,
													 kDNSServiceInterfaceIndexAny,
													 [name UTF8String],
													 [type UTF8String],
													 [domain UTF8String],
													 DNSServiceResolver::callback__,
													 nullptr);
...
void DNSServiceResolver::callback__(DNSServiceRef sdRef,
									DNSServiceFlags flags,
									uint32_t interfaceIndex,
									DNSServiceErrorType errorCode,
									const char *fullName,
		...
		if ([[dict valueForKey:@"data"] isKindOfClass:NSArray.class]) {
			...
			for (int index = 0; index < datas.count; index++) {
				if ([datas[index] isKindOfClass:NSDictionary.class]) {
					...
					if (cb_) {
						cb_([pid intValue], [signingId cStringUsingEncoding:NSUTF8StringEncoding],
