...
constexpr auto MINTIME = 60000;
constexpr auto HOUR = 60;

PLSLivingMsgItem::PLSLivingMsgItem(const QString &info, const long long time, pls_toast_info_type type, QWidget *parent) : QWidget(parent), m_msgInfo(info), m_times(time), m_type(type)
{
	ui = pls_new<Ui::PLSLivingMsgItem>();
	ui->setupUi(this);
...
{
	QString timeStr;
	if (qint64 timeOffset = (currentTime - m_times) / MINTIME; 0 == timeOffset) {
