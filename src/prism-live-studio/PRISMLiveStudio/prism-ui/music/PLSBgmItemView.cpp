...
{
	for (int i = (int)datas_.size() - 1; i >= 0; i--) {
		Insert(datas_[i], i);
	}
}
...
int PLSBgmItemViewModel::Count() const
{
	return static_cast<int>(datas.count());
}
...
PLSBgmItemViewModel::~PLSBgmItemViewModel() = default;
...
{
	if (CustomDataRole::DataRole == (CustomDataRole)role) {
		int row = index.row();
	...
	} else if (CustomDataRole::NeedPaintRole == (CustomDataRole)role) {
		return true;
...
void PLSBgmItemViewModel::doMediaStatusRole(const QModelIndex &index, const QVariant &value)
{
