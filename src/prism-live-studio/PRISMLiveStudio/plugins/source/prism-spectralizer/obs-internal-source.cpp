...
{
	auto s = static_cast<obs_internal_source *>(param);
	if (s)
		s->capture(src, data, muted);
}

obs_internal_source::obs_internal_source(config *cfg) : audio_source(), m_cfg(cfg)
{
...
obs_internal_source::~obs_internal_source()
{
	if (m_cfg->capture_source) {
		...
		if (source) {
			PLS_INFO("prism-spectralizer", "Removed audio capture from '%s'", obs_source_get_name(source));
			obs_source_remove_audio_capture_callback(source, audio_capture, this);
			obs_source_release(source);
		}
		obs_weak_source_release(m_cfg->capture_source);
		m_cfg->capture_source = nullptr;
	}
...
bool obs_internal_source::tick(float seconds)
{
	...
	std::string new_name = "";
	if (!m_cfg->capture_name.empty() && !m_cfg->capture_source) {
		uint64_t t = os_gettime_ns();

		if (t - m_capture_check_time > 3000000000) {
			new_name = m_cfg->capture_name;
	...
	if (!new_name.empty()) {
		...
		obs_weak_source_t *weak_capture = capture ? obs_source_get_weak_source(capture) : nullptr;

		if (!m_cfg->capture_name.empty() && new_name == m_cfg->capture_name) {
			m_cfg->capture_source = weak_capture;
			weak_capture = nullptr;
		}

		if (capture) {
			PLS_INFO("prism-spectralizer", "Added audio capture to '%s'", obs_source_get_name(capture));
...
bool obs_internal_source::copy_captured_data()
{
	/* Copy captured data */
...
void obs_internal_source::set_channels()
{
	std::lock_guard<std::mutex> lock(m_cfg->value_mutex);
	m_num_channels = (uint8_t)audio_output_get_channels(obs_get_audio());
}
...
void obs_internal_source::update()
{
	...
	if (m_cfg->audio_source_name.empty()) {
		if (m_cfg->capture_source) {
			old = m_cfg->capture_source;
			m_cfg->capture_source = nullptr;
		}
		m_cfg->capture_name = "";
	} else {
		if (m_cfg->capture_name.empty() || m_cfg->capture_name != m_cfg->audio_source_name) {
			if (m_cfg->capture_source) {
				old = m_cfg->capture_source;
				m_cfg->capture_source = nullptr;
			}
			m_cfg->capture_name = m_cfg->audio_source_name;
	...
	if (old) {
		...
		if (old_source) {
			PLS_INFO("prism-spectralizer", "Removed audio capture from '%s'", obs_source_get_name(old_source));
