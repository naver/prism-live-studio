...
spectrum_visualizer::spectrum_visualizer(config *cfg) : audio_visualizer(cfg), m_cfg(cfg)
{
	update();
}
...
void spectrum_visualizer::update()
{
	...
	if (params.rounded_corners) {
		...
		m_corner_radius = params.radius;
		struct vec2 offset = {};
		float angle = 180;
		for (int i = 0, l = 4 * params.corner_points; i <= l; i++) {
			offset.x = sin(RAD(angle)) * m_corner_radius;
			offset.y = cos(RAD(angle)) * m_corner_radius;
...
bool spectrum_visualizer::assign_bars_data(float seconds)
{
	visual_params params = m_cfg->vm_params[m_cfg->visual];
	if (m_sleeping) {
...
void spectrum_visualizer::tick(float seconds)
{
	...
	if (params.stereo) {
		is_silent_left = prepare_fft_input(m_cfg->buffer, m_cfg->sample_size, m_fftw_input_left, channel_mode::CM_LEFT);
		is_silent_right = prepare_fft_input(m_cfg->buffer, m_cfg->sample_size, m_fftw_input_right, channel_mode::CM_RIGHT);
	} else {
		is_silent_left = prepare_fft_input(m_cfg->buffer, m_cfg->sample_size, m_fftw_input_left, channel_mode::CM_LEFT);
	}
...
gs_vertbuffer_t *spectrum_visualizer::make_rounded_rectangle(float height, bool)
{
	...
	for (int i = 0; i < params.corner_points; i++) {
		auto &v = m_circle_points[i];
		auto &v1 = m_circle_points[i + 1];
		gs_vertex2f(v.x, v.y);
		gs_vertex2f(v1.x, v1.y);
		gs_vertex2f(0.5f, 0.5f);
	}

	auto diameter = params.bar_width;
	// bottom right
	for (int i = 10; i < params.corner_points * 2; i++) {
		auto &v = m_circle_points[i];
		auto &v1 = m_circle_points[i + 1];
		gs_vertex2f(v.x, v.y + height - diameter);
		gs_vertex2f(v1.x, v1.y + height - diameter);
		gs_vertex2f(0.5f, 0.5f);
	}

	auto offset = (params.bar_width % 2 == 0) ? 0 : 1;
	if (offset) {
		gs_vertex2f(0.0, -params.radius);
		gs_vertex2f(0.0, height - params.radius - (float)offset);
		gs_vertex2f((float)offset, height - params.radius - (float)offset);

		gs_vertex2f(0.0, -params.radius);
		gs_vertex2f((float)offset, height - params.radius - (float)offset);
		gs_vertex2f((float)offset, -params.radius);
	}

	// bottom left
	for (int i = 20; i < params.corner_points * 3; i++) {
		auto &v = m_circle_points[i];
		auto &v1 = m_circle_points[i + 1];
		gs_vertex2f(v.x + (float)offset, v.y + height - diameter);
		gs_vertex2f(v1.x + (float)offset, v1.y + height - diameter);
		gs_vertex2f(0.5f, 0.5f);
	}

	// top left
	for (int i = 30; i < params.corner_points * 4; i++) {
		auto &v = m_circle_points[i];
		auto &v1 = m_circle_points[i + 1];
		gs_vertex2f(v.x + (float)offset, v.y);
		gs_vertex2f(v1.x + (float)offset, v1.y);
		gs_vertex2f(0.5f, 0.5f);
	}
...
bool spectrum_visualizer::prepare_fft_input(const pcm_stereo_sample *buffer, uint32_t sample_size, double *fftw_input, channel_mode channel_mode) const
{
	...
	for (auto i = 0u; i < sample_size; ++i) {
		switch (channel_mode) {
		case channel_mode::CM_LEFT:
			fftw_input[i] = buffer[i].l;
			break;
		case channel_mode::CM_RIGHT:
			fftw_input[i] = buffer[i].r;
			break;
		case channel_mode::CM_BOTH:
...
void spectrum_visualizer::smooth_bars(doublev *bars)
{
	...
	case smooting_mode::SM_MONSTERCAT:
		monstercat_smoothing(bars);
		break;
	case smooting_mode::SM_SGS:
		sgs_smoothing(bars);
		break;
	default:;
	}
...
void spectrum_visualizer::monstercat_smoothing(doublev *bars)
{
	...
	for (auto i = 1L; i < bars_length; ++i) {
		...
		if ((*bars)[outer_index] < params.bar_min_height) {
			...
			monstercat_calculate_weight(bars, i);
		}
	}
}

void spectrum_visualizer::monstercat_calculate_weight(doublev *bars, int index_)
{
	auto bars_length = static_cast<int64_t>(bars->size());
	auto outer_index = static_cast<size_t>(index_);

	for (int64_t j = 0; j < bars_length; ++j) {
		if (index_ != j) {
			const auto index = static_cast<size_t>(j);
			const auto weighted_value = (*bars)[outer_index] / m_monstercat_smoothing_weights[static_cast<size_t>(std::abs(index_ - j))];
...
void spectrum_visualizer::calculate_moving_average_and_std_dev(double new_value, size_t max_number_of_elements, doublev *old_values, double *moving_average, double *std_dev) const
{
	...
	*moving_average = sum / (double)old_values->size();

	auto squared_summation = std::inner_product(old_values->begin(), old_values->end(), old_values->begin(), 0.0);
	*std_dev = std::sqrt((squared_summation / (double)old_values->size()) - std::pow(*moving_average, 2));
}
...
void spectrum_visualizer::maybe_reset_scaling_window(double current_max_height, size_t max_number_of_elements, doublev *values, double *moving_average, double *std_dev) const
{
	const auto reset_window_size = (constants::auto_scaling_reset_window * (double)max_number_of_elements);
...
void spectrum_visualizer::create_spectrum_bars(fftw_complex *fftw_output, size_t fftw_results, int32_t win_height, uint32_t number_of_bars, doublev *bars, const doublev *bars_falloff)
{
	UNUSED_PARAMETER(bars_falloff);
...
void spectrum_visualizer::recalculate_cutoff_frequencies(uint32_t number_of_bars, uint32v *low_cutoff_frequencies, uint32v *high_cutoff_frequencies, doublev *freqconst_per_bin) const
{
	...
	for (auto i = 0u; i <= number_of_bars; i++) {
		(*freqconst_per_bin)[i] = m_cfg->high_cutoff_freq * std::pow(10.0, (freq_const * -1) + (((i + 1.0) / (number_of_bars + 1.0)) * freq_const));
					...
					doublev *bars) const
{
	...
	for (auto i = 0u; i < number_of_bars; i++) {
		...
		/* boost high freqs */
		(*bars)[i] *= (std::log2(2 + i) * (100.f / (float)number_of_bars));
