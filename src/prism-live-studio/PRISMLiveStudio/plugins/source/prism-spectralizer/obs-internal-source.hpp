...
class obs_internal_source : public audio_source {
	...
	std::array<circlebuf, 2> m_audio_data{}; /* Left & Right data from capture callback */
	std::array<float *, 2> m_audio_buf{};    /* Copy of captured audio */
	...
	bool copy_captured_data();

	void set_channels();

public:
	explicit obs_internal_source(config *cfg);
	~obs_internal_source() override;

	obs_internal_source(obs_internal_source const &) = delete;
	obs_internal_source &operator=(obs_internal_source const &) = delete;

	obs_internal_source(obs_internal_source &&) = delete;
	obs_internal_source &operator=(obs_internal_source &&) = delete;
