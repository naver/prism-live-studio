...
static const char *const s_default_font = "Academy Engraved LET";
#if defined(Q_OS_WIN)
...
static inline void colorIntList(QString &colorList, const QStringList &colorsData)
{
	QStringList tmpList;
...
static void attachTMInfo(bool isForce = false)
{
	if (!g_TMTemplateJsons.isEmpty() && !isForce)
		return;

	g_TMTemplateJsons.clear();
	auto lang = pls_get_locale() != "ko-KR" ? "en" : "ko";
	auto groups = pls::rsm::getResourceManager()->getGroups(PLS_RSM_CID_TEXT_TEMPLATE);
	pls::rsm::Category category;
	if (groups.empty()) {
		category = pls::rsm::getResourceManager()->getDefaultCategory(PLS_RSM_CID_TEXT_TEMPLATE);
		groups = category.groups();
	}
	PLS_DEBUG("text-template", "text-template source group is %d", groups.size());

	for (auto group : groups) {
		auto items = group.items();
		PLS_DEBUG("text-template", "text-template source item is %d", items.size());

		for (auto item : items) {
			auto itemAttrObj = item.attr("properties").toHash();
			auto contentObj = itemAttrObj.value("diaplayDefaultText").toHash();
			auto characterObj = itemAttrObj.value("characterProperties").toHash();

			TMTemplateAttr templateAttr;
			templateAttr.isPaid = false;
			templateAttr.name = item.attr("title").toString();
			templateAttr.firstText = contentObj.value("defaultText1").toHash().value(lang).toString();
			templateAttr.secText = contentObj.value("defaultText2").toHash().value(lang).toString();
			templateAttr.textCount = templateAttr.secText.isEmpty() ? 1 : 2;
			templateAttr.isFontSettings = characterObj.value("isDisplay").toBool();
			templateAttr.isFont = characterObj.value("font").toHash().value("isDisplay").toBool();
#if defined(Q_OS_WIN)
			templateAttr.fontFamily = characterObj.value("font").toHash().value("family").toHash().value(lang).toString().simplified();
			templateAttr.fontWeight = characterObj.value("font").toHash().value("style").toHash().value(lang).toString().simplified();
#elif defined(Q_OS_MACOS)
			templateAttr.fontFamily = s_default_font;
			templateAttr.fontWeight = "Bold";
#endif

			templateAttr.isHorizontalAlignment = characterObj.value("horizontalAlignment").toHash().value("isDisplay").toBool();
			templateAttr.hAlignValue = getTextAlignIndex(characterObj.value("horizontalAlignment").toHash().value("alignmentStyle").toString());
			templateAttr.isSize = characterObj.value("fontSize").toHash().value("isDisplay").toBool();

			templateAttr.fontSizeValue = characterObj.value("fontSize").toHash().value("defaultSizePt").toInt();
			templateAttr.isBoxSize = characterObj.value("boxSize").toHash().value("isDisplay").toBool();
			templateAttr.letterValue = getLetterIndex(characterObj.value("upCase").toHash().value("style").toString());
			templateAttr.isLetter = characterObj.value("upCase").toHash().value("isDisplay").toBool();
			auto textColorObj = itemAttrObj.value("colorProperties").toHash().value("textColor").toHash();
			auto outLineColorObj = itemAttrObj.value("colorProperties").toHash().value("outLineColor").toHash();
			auto bgColorObj = itemAttrObj.value("colorProperties").toHash().value("BackgroudColor").toHash();

			templateAttr.isColorSettings = itemAttrObj.value("colorProperties").toHash().value("isDisplay").toBool();
			...
			templateAttr.textTransparentValue = textColorObj.value("colorAlpha").toInt();
			colorIntList(templateAttr.textcolorList, textColorObj.value("defaultColorRGBCodeList").toStringList());
			...
			templateAttr.backgroundTransparentValue = bgColorObj.value("colorAlpha").isNull() ? 100 : bgColorObj.value("colorAlpha").toInt();

			colorIntList(templateAttr.backgroundColorList, bgColorObj.value("defaultColorRGBCodeList").toStringList());
			...
			templateAttr.strokeLineValue = outLineColorObj.value("outLineSizePt").isNull() ? 1 : outLineColorObj.value("outLineSizePt").toInt();

			auto motionObj = itemAttrObj.value("motionProperties").toHash();

			templateAttr.isMotionSettings = itemAttrObj.value("motionProperties").toHash().value("isDisplay").toBool();
			templateAttr.isRotate = motionObj.value("isRepeat").toBool();
			templateAttr.isMotion = motionObj.value("isDisplay").toBool();

			templateAttr.motionValue = motionObj.value("defaultmotionSpeed").isNull() ? 50 : motionObj.value("defaultmotionSpeed").toInt();
			auto itemId = getItemId(item.attr("itemId").toString());
			g_TMTemplateJsons.insert(itemId, templateAttr);
			if (templateAttr.isPaid) {
				s_paidIds.insert(itemId);
			}
		}
	}
}
...
void set_tm_text_content_data(obs_data_t *settings, const TMTemplateAttr &tm_attr, bool is_default)
{
	...
	if (is_default) {
		obs_data_set_bool(tm_text_content_obj, "text-content-change-1", false);
		obs_data_set_bool(tm_text_content_obj, "text-content-change-2", false);
...
void set_tm_text_color_data(obs_data_t *settings, const TMTemplateAttr &tm_attr, bool is_default)
{
	...
	if (is_default) {
		obs_data_set_bool(tm_color, "text-color-change", false);
		obs_data_set_int(tm_color, "color-tab", 0);
...
{
	PLS_INFO("", "tm_template_list_changed");
	attachTMInfo();
	...
	if ((context->currentTemplateListIndex == index) || isFirstShow) {
		...
		if (g_TMTemplateJsons.find(index) != g_TMTemplateJsons.end()) {
			...
			QString url = QUrl::fromLocalFile(pls::rsm::getAppDataPath("textTemplatePC/web/index.html")).toString();
			process_url_language(url);
			obs_data_set_string(browser_settings, "url", url.toUtf8().constData());

			context->m_allowBrowserComposite.store(false);
...
void text_template_source_get_defaults(obs_data_t *settings)
{
	...
	if (g_TMTemplateJsons.isEmpty()) {
		PLS_INFO("text-template", "template json is null");
		return;
	}
...
static void source_notified(void *data, calldata_t *calldata)
{
	...
	case OBS_SOURCE_BROWSER_LOADED:
		context->m_allowBrowserComposite.store(true);
...
static void *text_template_source_create(obs_data_t *settings, obs_source_t *source)
{
	...
	if (!context) {
		PLS_PLUGIN_ERROR("textmotion source create failed, because out of memory.");
...
static void text_template_source_render(void *data, obs_source_t *source)
{
	...
	if (vc_source->m_allowBrowserComposite.load())
...
static void check_source_is_paid(void *data, obs_data_t *private_data)
{
	...
	obs_data_set_bool(private_data, API_PAID_KEY_NAME, s_paidIds.find(obs_data_get_int(source_settings, S_TM_TEMPLATE_LIST)) != s_paidIds.end());
...
void text_template_source::update(obs_data_t *settings, bool isForce)
{
	...
	if (!pls_object_is_valid(this) || m_bIsDestroyed) {
		return;
	}
	type.clear();
	textColor.clear();
	...
	if (settings) {
		...
		OBSDataAutoRelease tmTextVal = obs_data_get_obj(settings, S_TM_TEXT);
		OBSDataAutoRelease tmContentVal = obs_data_get_obj(settings, S_TM_TEXT_CONTENT);
		OBSDataAutoRelease tmColorVal = obs_data_get_obj(settings, S_TM_COLOR);
		OBSDataAutoRelease tmMotionVal = obs_data_get_obj(settings, S_TM_MOTION);
		if (tmTextVal && tmContentVal && tmColorVal && tmMotionVal) {
	...
	if (!updateTextTemplateInfo.isEmpty()) {
		...
		PLS_PLUGIN_DEBUG("params is empty");
	}
}
...
{
	QByteArray json = toJsonStr(true).toUtf8();
	pls_source_update_extern_params_json(m_source, json.constData(), sub_code);
}
...
void text_template_source::updateExternParams(const QByteArray &cjson, int sub_code)
{
	...
	case OBS_SOURCE_TEXT_TEMPLATE_UPDATE_PARAMS_SUB_CODE_SIZECHANGED:
		updateBoxSize(cjson);
		break;
	case OBS_SOURCE_TEXT_TEMPLATE_UPDATE_PARAMS_SUB_CODE_JSONLOADED: {
