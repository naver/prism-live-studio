...
#define do_log(level, format, ...) plog(level, "[x265 encoder: '%s' %p] " format, obs_encoder_get_name(obj->obs_encoder), obj->obs_encoder, ##__VA_ARGS__)
...
{
	const struct prism_x265 *obj = data;
...
static bool update_settings(struct prism_x265 *obj, obs_data_t *settings)
{
	const video_t *video = obs_encoder_video(obj->obs_encoder);
	...
	if (profile && (*profile) && (obj->x265_api->param_apply_profile(obj->x265_params, profile) < 0)) {
...
static void *prism_x265_create(obs_data_t *settings, obs_encoder_t *encoder)
{
	...
	do {
		...
		if (!update_settings(obj, settings)) {
...
static bool prism_x265_update(void *data, obs_data_t *settings)
{
	...
	bool success = update_settings(obj, settings);
	if (!success) {
...
static bool prism_x265_encode(void *data, struct encoder_frame *frame, struct encoder_packet *packet, bool *received_packet)
{
	...
	if (ret < 0) {
		obs_send_encoder_error(obj->obs_encoder, "x265_api->encoder_encode", ret);
