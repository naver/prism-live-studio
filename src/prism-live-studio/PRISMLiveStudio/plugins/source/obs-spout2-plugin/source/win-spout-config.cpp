...
win_spout_config::win_spout_config() : auto_start(false), spout_output_name("PRISM_Spout")
{
	config_t *obs_config = obs_frontend_get_global_config();

	if (obs_config) {
		config_set_default_bool(obs_config, SECTION_NAME, PARAM_AUTO_START, auto_start);
		config_set_default_string(obs_config, SECTION_NAME, PARAM_SPOUT_OUTPUT_NAME, spout_output_name.toUtf8().constData());
	}
}
...
void win_spout_config::save()
{
	...
	if (obs_config) {
		...
		config_set_string(obs_config, SECTION_NAME, PARAM_SPOUT_OUTPUT_NAME, spout_output_name.toUtf8().constData());
