...
//------------------------------------------------------------------------
PLSDuplicatorCore::PLSDuplicatorCore(DuplicatorType duplicator_type, int adapter_index, int adapter_output_index) : type(duplicator_type), adapter(adapter_index), adapter_output(adapter_output_index)
{
}
...
bool PLSDuplicatorCore::init()
{
	...
	if (FAILED(hr)) {
		PLS_ERROR("monitor capture", "Failed to get IDXGIOutput1, error:0X%X", hr);
		assert(false);
		return false;
	}
	...
	PLS_INFO("monitor capture", "Duplicator is inited. ad:%d output:%d", adapter, adapter_output);
	inited = true;
	return true;
}
...
bool PLSDuplicatorCore::check_update_texture()
{
	...
	if (!inited || !update_texture()) {
		...
		if (!init())
			...
			PLS_INFO("monitor capture", "Success to reset duplicator");
	}

	return !!monitor_texture.Get();
}

bool PLSDuplicatorCore::is_size_changed(const gs_texture *gs_frame) const
{
	if (!gs_frame) {
		assert(false);
		return true;
...
bool PLSDuplicatorCore::upload_memory_texture(gs_texture *&gs_frame)
{
	...
	assert(type == DuplicatorType::DUPLICATOR_MEMORY_COPY);
	if (!inited || !monitor_texture.Get() || type != DuplicatorType::DUPLICATOR_MEMORY_COPY)
		return false;
	...
	if (!gs_frame || is_size_changed(gs_frame)) {
		...
		gs_frame = gs_texture_create(texture_desc.Width, texture_desc.Height, GS_BGRA, 1, nullptr, GS_DYNAMIC);
		if (!gs_frame)
			return false;
	}
...
bool PLSDuplicatorCore::upload_shared_texture(gs_texture *&gs_frame, HANDLE &handle_output)
{
	...
	assert(type == DuplicatorType::DUPLICATOR_SHARED_HANDLE);
	if (!inited || !monitor_texture.Get() || type != DuplicatorType::DUPLICATOR_SHARED_HANDLE)
		return false;
	...
	if (!gs_frame || is_size_changed(gs_frame) || handle_output != shared_handle) {
		if (gs_frame) {
			gs_texture_destroy(gs_frame);
			gs_frame = nullptr;
			handle_output = nullptr;
		}

		gs_frame = gs_texture_open_shared(uint32_t(uint64_t(shared_handle)));
		if (!gs_frame)
			return false;
...
bool PLSDuplicatorCore::copy_texture(ComPtr<IDXGIResource> res, const DXGI_OUTDUPL_FRAME_INFO &info)
{
...
bool PLSDuplicatorCore::init_device(IDXGIAdapter *pAdapter)
{
	...
	HRESULT hr = D3D11CreateDevice(pAdapter, D3D_DRIVER_TYPE_UNKNOWN, nullptr, D3D11_CREATE_DEVICE_BGRA_SUPPORT, featureLevels.data(), (UINT)featureLevels.size(), D3D11_SDK_VERSION,
...
bool PLSDuplicatorCore::init_output(int monitor_idx, IDXGIOutput **dxgiOutput) const
{
...
bool PLSDuplicatorCore::create_read_texture(D3D11_TEXTURE2D_DESC &td)
{
	...
	HRESULT hr = d3d11_device->CreateTexture2D(&td, nullptr, texture.Assign());
	if (FAILED(hr)) {
		PLS_ERROR("monitor capture", "create dump texture failed, error:0X%X", hr);
		return false;
	}
...
bool PLSDuplicatorCore::create_shared_texture(const D3D11_TEXTURE2D_DESC &descTemp)
{
	DXGI_FORMAT fmt;
	...
	ComPtr<ID3D11Texture2D> texture;
	HRESULT hr = d3d11_device->CreateTexture2D(&desc, nullptr, texture.Assign());
	if (!texture) {
		PLS_ERROR("monitor capture", "create shared texture failed, error:0X%X", hr);
		assert(false);
		return false;
	}

	ComPtr<IDXGIResource> dxgi_res;
	texture->QueryInterface(__uuidof(IDXGIResource), (void **)dxgi_res.Assign());
	if (!dxgi_res) {
		assert(false);
		return false;
	}
