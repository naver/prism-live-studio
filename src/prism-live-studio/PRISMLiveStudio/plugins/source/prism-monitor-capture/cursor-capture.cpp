...
bool PLSCursorCapture::cursor_capture_icon(HICON icon)
{
	...
	if (width != 0 && height != 0 && (width != cursor_info.width || height != cursor_info.height)) {
		...
		cursor_info.bitmap = (uint8_t *)(bzalloc(width * height * 4));
...
uint8_t *PLSCursorCapture::copy_from_mask(ICONINFO *ii, uint32_t &width, uint32_t &height)
{
	...
	output = (uint8_t *)(bzalloc(pixels * 4));
...
uint8_t *PLSCursorCapture::get_bitmap_data(HBITMAP hbmp, BITMAP *bmp)
{
	if (GetObject(hbmp, sizeof(*bmp), bmp) != 0) {
		uint8_t *output;
		//PRISM/WangShaohui/20220119/#11212/variable overflow
		size_t size = ((size_t)bmp->bmHeight * (size_t)bmp->bmWidth * (size_t)bmp->bmBitsPixel) / 8;

		output = (uint8_t *)(bmalloc(size));
		GetBitmapBits(hbmp, static_cast<LONG>(size), output);
		return output;
	}
...
uint8_t PLSCursorCapture::bit_to_alpha(const uint8_t *data, long pixel, bool invert) const
{
...
bool PLSCursorCapture::bitmap_has_alpha(const uint8_t *data, long num_pixels) const
{
...
void PLSCursorCapture::apply_mask(uint8_t *color, const uint8_t *mask, long num_pixels) const
{
	for (long i = 0; i < num_pixels; i++)
...
bool PLSCursorCapture::update_texture()
{
	...
	if (!texture || texture_width != cursor_info.width || texture_height != cursor_info.height) {
		gs_texture_destroy(texture);
		texture = gs_texture_create(cursor_info.width, cursor_info.height, GS_BGRA, 1, nullptr, GS_DYNAMIC);
