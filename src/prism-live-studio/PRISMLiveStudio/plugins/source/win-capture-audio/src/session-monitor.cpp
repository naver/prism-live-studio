	...
	: session_control{session_control}
{
	...
	info("registered new session: [%d] %s", pid, executable.c_str());
}

SessionWatcher::~SessionWatcher()
{
	session_control->UnregisterAudioSessionNotification(&notification_client.value());
	info("session expired: [%d] %s", pid, executable.c_str());
}
...
void SessionMonitor::AddDevice(std::wstring id, wil::com_ptr<IMMDevice> device)
{
	...
	info("registered new device: %ls", id.c_str());
}
...
void SessionMonitor::RemoveDevice(std::wstring id)
{
	...
	info("removed device: %ls", id.c_str());
}
...
void SessionMonitor::AddSession(MSG msg)
{
	...
	*session_control.put() = session_control_ptr;
	...
	info("Session is added. pid: %u, exe: %s", pid, get_exe_by_pid(pid).c_str());
...
void SessionMonitor::RemoveSession(MSG msg)
{
	...
	*session_control.put() = session_control_ptr;
	...
	//PRISM/WangShaohui/20220314/none/modify log
	info("Session is removed. pid: %u", pid);
...
void SessionMonitor::SafeRun()
{
	try {
		...
		warn("Exception in session monitor. %s", e.what());
	}
}
