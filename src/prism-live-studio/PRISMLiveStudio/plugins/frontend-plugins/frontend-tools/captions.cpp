...
CaptionsDialog::CaptionsDialog(QWidget *parent) : PLSDialogView(parent), ui(new Ui_CaptionsDialog)
{
	...
	this->setupUi(ui);
	pls_add_css(this, {"CaptionsDialog"});
	initSize(720, 700);
	...
	connect(ui->accept, &QPushButton::clicked, this, &CaptionsDialog::accept);
}
...
static void get_valid_locale_names(vector<locale_info> &locales)
{
	...
	static const LANGID default_locales[] = {0x0409, 0x0401, 0x0402, 0x0403, 0x0404, 0x0405, 0x0406, 0x0407, 0x0408, 0x040a, 0x040b, 0x040c, 0x040d, 0x040e,
						 0x040f, 0x0410, 0x0411, 0x0412, 0x0413, 0x0414, 0x0415, 0x0416, 0x0417, 0x0418, 0x0419, 0x041a, 0};
