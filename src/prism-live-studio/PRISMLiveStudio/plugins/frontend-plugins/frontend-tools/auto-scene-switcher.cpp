...
SceneSwitcher::SceneSwitcher(QWidget *parent) : PLSDialogView(parent), ui(new Ui_SceneSwitcher)
{
	...
	if (!switcher) {
		PLS_WARN("SceneSwitcher", "switcher is null");
...
extern "C" void FreeSceneSwitcher()
{
	...
	if (switcher) {
		PLS_INFO("SceneSwitcher", "FreeSceneSwitcher");
		delete switcher;
		switcher = nullptr;
	}
}
...
extern "C" void InitSceneSwitcher()
{
	...
	switcher = new SwitcherData;
	PLS_INFO("SceneSwitcher", "InitSceneSwitcher");
	if (!switcher) {
		PLS_ERROR("SceneSwitcher", "new SwitcherData failed");
		return;
	}
