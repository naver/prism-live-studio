...
DNSSDRegistrationImpl::DNSSDRegistrationImpl()
: self(nullptr)
{
}
...
void DNSSDRegistrationImpl::init(const std::string &domain,
                                 ...
                                 const std::map<std::string, std::string> &txtRecords)
{
    ...
    for (const auto& it : txtRecords){
        NSString *key = [NSString stringWithCString:it.first.c_str()
                                           encoding:[NSString defaultCStringEncoding]];
        NSString *value = [NSString stringWithCString:it.second.c_str()
                                           encoding:[NSString defaultCStringEncoding]];
        txtDicts[key] = value;
    }
...
- (id)initWithDomain:(NSString *)domain
                ...
                name:(NSString *)name
                port:(NSUInteger)port
          txtRecords:(NSDictionary *)txtRecords
{
...
- (void)registerService
{
    ...
    errorCode = DNSServiceRegister(&self->sdRef_,
                                   0,
    ...
    if (errorCode == kDNSServiceErr_NoError) {
        ...
        [self stopWithError:[NSError errorWithDomain:NSNetServicesErrorDomain
                                                code:errorCode
                                            userInfo:nil] notify:YES];
    }
}
