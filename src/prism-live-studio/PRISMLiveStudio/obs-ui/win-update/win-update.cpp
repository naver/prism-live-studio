...
try {
	...
	BPtr<char> updateFilePath =
		GetConfigPathPtr("PRISMLiveStudio\\updates\\updater.exe");
...
try {
	...
	if (FetchAndVerifyFile("branches", "PRISMLiveStudio\\updates\\branches.json",
	...
	if (!FetchAndVerifyFile("manifest",
				"PRISMLiveStudio\\updates\\manifest.json",
...
try {
	...
	if (FetchAndVerifyFile("whatsnew", "PRISMLiveStudio\\updates\\whatsnew.json",
