...
OAuthLogin::OAuthLogin(QWidget *parent, const std::string &url, bool token) : QDialog(parent), get_token(token)
{
	...
	connect(cefWidget, SIGNAL(titleChanged(QString)), this, SLOT(setWindowTitle(QString)));
	connect(cefWidget, SIGNAL(urlChanged(QString)), this, SLOT(urlChanged(QString)));
