...
void ColorSourceToolbar::on_choose_clicked()
{
	...
	QColor newColor =
		PLSColorDialogView::getColor(color, this, desc, options);
	if (!newColor.isValid()) {
		return;
	}
	...
	: SourceToolbar(parent, source), ui(new Ui_TextSourceToolbar)
{
	...
	setStyleSheet(
		"TextSourceToolbar QPushButton {   padding: 0px; margin: 0px; min-width: 89px;}");
...
void TextSourceToolbar::on_selectFont_clicked()
{
	...
	font = PLSFontDialogView::getFont(&success, font, this, "Pick a Font",
					  options);
	if (!success) {
		return;
	}
