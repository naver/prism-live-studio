	  ...
	  noPreviewMargin(13)
{
	...
	ui->asyncFilters->setItemDelegate(new PLSFiltersItemDelegate(ui->asyncFilters));
	ui->effectFilters->setItemDelegate(new PLSFiltersItemDelegate(ui->effectFilters));
	...
	connect(ui->property_buttonBox->button(QDialogButtonBox::RestoreDefaults), &QPushButton::clicked, this,
...
void OBSBasicFilters::UpdatePropertiesView(int row, bool async)
{
	...
	if (pls_is_equal(id, "vst_filter")) {
		...
		view = new PLSPropertiesView(settings.Get(), filter, (PropertiesReloadCallback)obs_source_properties,
...
{
	if (auto view = dynamic_cast<PLSPropertiesView *>(static_cast<OBSBasicFilters *>(data)->view); view) {
		...
		if (filterItem == filter) {
			if ((int)idx != i) {
				...
				if (listItem) {
					...
					PLSSetupVisibilityItem(list, listItem, filterItem);
...
void OBSBasicFilters::ResetFilters()
{
	...
	if (pls_is_equal("sound_touch_filter", id)) {
		...
		obs_data_set_string(data, "method", "set_default");
	...
	if (auto filterView = dynamic_cast<PLSPropertiesView *>(view); filterView) {
		if (!filterView->DeferUpdate())
			obs_source_update(filter, nullptr);

		filterView->ReloadProperties();
	} else if (auto filterView = dynamic_cast<PLSVstFilterView *>(view); filterView) {
