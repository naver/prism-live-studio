...
void ScreenshotObj::Screenshot()
{
	...
	if (gs_texrender_begin_with_color_space(texrender, cx, cy, space)) {
		...
		if (source) {
			...
			isVerticalPreview ? pls_source_video_render_vertical(source)
					  : pls_source_video_render_landscape(source);
			obs_source_dec_showing(source);
		} else {
			isVerticalPreview ? pls_render_vertical_main_texture() : obs_render_main_texture();
		}
...
void ScreenshotObj::MuxAndFinish()
{
	...
	emit Finished(QString::fromStdString(path));
	deleteLater();
}

void ScreenshotObj::Start(bool showTipOnFinished)
{
...
void OBSBasic::ScreenshotSelectedSource()
{
	...
	if (isVerticalPreview) {
		item = PLSSceneitemMapMgrInstance->getVerticalSelectedSceneitem(item);
	}
