...
static OBSOutputAutoRelease create_output()
{
	...
	blog(LOG_INFO, "%s-%p output created by PRISM.", __FUNCTION__, output.Get());
	if (!output) {
						       ...
						       bool &enableDynBitrate)
{
	...
	auto strText = QString(QTStr("MultitrackVideo.IncompatibleSettings.Text"))
			       .arg(obs_data_get_string(service_settings, "multitrack_video_name"))
			       .arg(incompatible_settings)
			       .arg(where_to_disable);

	auto clickedButton = PLSAlertView::question(
		parent, QTStr("MultitrackVideo.IncompatibleSettings.Title"), strText,
		{{PLSAlertView::Button::Yes, QTStr("MultitrackVideo.IncompatibleSettings.DisableAndStartStreaming")},
		 {PLSAlertView::Button::YesToAll,
		  QTStr("MultitrackVideo.IncompatibleSettings.UpdateAndStartStreaming")},
		 {PLSAlertView::Button::Cancel, QTStr("Cancel")}},
		PLSAlertView::Button::Cancel);

	const char *action = "cancel";
	if (clickedButton == PLSAlertView::Button::Yes) {
		action = "DisableAndStartStreaming";
	} else if (clickedButton == PLSAlertView::Button::YesToAll) {
	...
	if (clickedButton == PLSAlertView::Button::Yes || clickedButton == PLSAlertView::Button::YesToAll) {
		useDelay = false;
		enableNewSocketLoop = false;
		enableDynBitrate = false;

		if (clickedButton == PLSAlertView::Button::YesToAll) {
