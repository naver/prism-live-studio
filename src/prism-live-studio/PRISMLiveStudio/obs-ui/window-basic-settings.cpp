...
void OBSBasicSettings::LoadHotkeySettings(obs_hotkey_id ignoreKey)
{
	...
	hotkeysLayout->setLabelAlignment(Qt::AlignRight | Qt::AlignTrailing |
					 Qt::AlignVCenter);
	hotkeysLayout->setProperty("ZeroVerticalSpacing", QVariant(true));
...
void OBSBasicSettings::SaveSettings()
{
	...
	qobject_cast<PLSBasic *>(main)->showEncodingInStatusBar();
}
...
void OBSBasicSettings::on_advOutRecEncoder_currentIndexChanged(int idx)
{
	...
	auto setRescaleVisible = [=](bool visible) {
		if (visible) {
			ui->formLayout_16->setLayout(6, QFormLayout::LabelRole,
						     ui->horizontalLayout_24);
			ui->formLayout_16->setWidget(
				6, QFormLayout::FieldRole,
			...
			ui->horizontalLayout_24->setAlignment(Qt::AlignLeft |
							      Qt::AlignVCenter);
			ui->advOutRecUseRescale->show();
			ui->advOutRecRescaleContainer->show();
			ui->label_27->show();
		} else {
			layoutRemoveWidget(ui->formLayout_16,
					   ui->advOutRecUseRescale);
			layoutRemoveWidget(ui->formLayout_16,
					   ui->advOutRecRescaleContainer);
			layoutRemoveWidget(ui->formLayout_16, ui->label_27);
...
void OBSBasicSettings::SpeakerLayoutChanged(int idx)
{
	...
	if (surround) {
		...
		updateAlertMessage(
			AlertMessageType::Warning, ui->channelSetup,
			QTStr("Basic.Settings.Audio.MultichannelWarning.Enabled") +
				QStringLiteral("\n\n") +
				QTStr("Basic.Settings.Audio.MultichannelWarning"));
		PopulateAACBitrates(
...
void OBSBasicSettings::AdvOutRecCheckWarnings()
{
	...
	if (recEncoder.contains("ProRes")) {
		if (recFormat.compare("mkv") == 0) {
			...
			updateAlertMessage(
				AlertMessageType::Warning, ui->advOutRecFormat,
			    ...
			    const QWidget *widget, const QWidget *advCurrentTab)
{
	if (page == ui->generalPage) {
		...
		return ui->listWidget->currentRow() == 6;
	} else if (page == ui->advancedPage) {
		return ui->listWidget->currentRow() == 7;
	} else {
		return false;
	}
}
