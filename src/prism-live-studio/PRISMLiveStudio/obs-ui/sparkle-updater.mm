...
- (SUAppcastItem *)bestValidUpdateWithDeltasInAppcast:(SUAppcast *)appcast forUpdater:(SUUpdater *)updater
{
    ...
    NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithDictionary:item.propertiesDictionary];
    NSString *build = [host objectForInfoDictionaryKey:@"CFBundleVersion"];
    NSString *url = dict[@"sparkle:releaseNotesLink"];
    dict[@"sparkle:releaseNotesLink"] = [url stringByAppendingFormat:@"#%@", build];
...
{
    SUAppcastItem *selected = [self bestValidUpdateWithDeltasInAppcast:appcast forUpdater:updater];
...
- (id<SUVersionComparison>)versionComparatorForUpdater:(SUUpdater *)__unused updater
{
    return self;
}
...
static inline bool bundle_matches(NSBundle *bundle)
{
    ...
    return [bundle.bundleIdentifier isEqual:@"com.obsproject.obs-studio"] && r.location != NSNotFound;
}
