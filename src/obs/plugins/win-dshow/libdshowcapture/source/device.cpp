...
HDevice::~HDevice()
{
	...
	//PRISM/WangShaohui/20200117/#281/for source unavailable
	if (exitEvt && exitEvt != INVALID_HANDLE_VALUE) {
		CloseHandle(exitEvt);
		exitEvt = 0;
	}
}
...
void HDevice::EventThreadInner()
{
	...
	while (true) {
		...
		if (res == WAIT_OBJECT_0)
			break; // exitEvt is set for breaking thread
...
void HDevice::StartEventThread()
{
	...
	if (exitEvt && exitEvt != INVALID_HANDLE_VALUE) {
		::ResetEvent(exitEvt);
		thread = (HANDLE)_beginthreadex(0, 0, EventThread, this, 0, 0);
	}
}
...
bool HDevice::CreateGraph()
{
	...
	//PRISM/WangShaohui/20200117/#281/for source unavailable
	if (!CreateFilterGraph(&graph, &builder, &control, &events))
		return false;

	//PRISM/WangShaohui/20200117/#281/for source unavailable
	if (events.Get()) {
		HRESULT hr = events->GetEventHandle((OAEVENT *)&msgEvt);
		if (FAILED(hr))
...
Result HDevice::Start()
{
	...
	if (FAILED(hr)) {
		...
		if (hr == (HRESULT)0x800705aa || hr == (HRESULT)0x8007001F) {
