...
static bool is_slideshow_valid(struct slideshow *ss)
{
	bool valid = false;
	...
	if (ss->files.num <= 0) {
		...
		for (size_t i = 0; i < ss->files.num; ++i) {
			struct image_file_data item = ss->files.array[i];
			if (item.source &&
			    obs_source_get_capture_valid(item.source, NULL)) {
				valid = true;
				break;
			}
		}
...
static void ss_video_render(void *data, gs_effect_t *effect)
{
	...
	//PRISM/WangShaohui/20200117/#281/for source unavailable
	if (is_slideshow_valid(ss)) {
		obs_source_set_capture_valid(ss->source, true,
					     OBS_SOURCE_ERROR_OK);
	} else {
		obs_source_set_capture_valid(ss->source, false,
