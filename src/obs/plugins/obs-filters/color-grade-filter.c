...
static void color_grade_filter_update(void *data, obs_data_t *settings)
{
	...
	double clut_amount = obs_data_get_int(settings, SETTING_CLUT_AMOUNT);
	...
	filter->clut_amount = (float)clut_amount / 100;
...
{
	//PRISM/Liuying/20200317/prism color filter ux: filter density range value: 0-100%
	obs_data_set_default_double(settings, SETTING_CLUT_AMOUNT, 100);
}
