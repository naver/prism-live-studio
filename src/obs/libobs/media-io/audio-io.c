...
//PRISM/LiuHaibin/20200213/#214/for outro
...
/* ------------------------------------------------------------------------- */
//PRISM/LiuHaibin/20200213/#214/for outro
static inline void do_audio_output_for_outro(struct audio_output *audio,
					     uint64_t timestamp,
					     uint32_t frames,
					     obs_outro_t *outro)
{
			     ...
			     uint64_t prev_time)
{
	...
	if (obs_outro_active(outro)) {
		do_audio_output_for_outro(audio, new_ts, AUDIO_OUTPUT_FRAMES,
					  outro);
		return;
	}
	//End
