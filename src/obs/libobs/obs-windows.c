...
static void log_security_products_by_type(IWSCProductList *prod_list, int type)
{
	...
	for (int i = 0; i < count; i++) {
		...
		//OBS Modification:
		//Zhang Dewen / 20200305 / Related Issue 807
		//Reason: name is not convert to utf8 from wchar(unicode)
		//Solution: log encode error
		int name_len = 1 + WideCharToMultiByte(CP_UTF8, 0, name, -1,
						       NULL, 0, NULL, NULL);
