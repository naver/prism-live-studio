...
void SourceTree::Remove(OBSSceneItem item)
{
	...
	if (!main->SavingDisabled()) {
		...
		plog(LOG_INFO, "User Removed source '%s' (%s) from scene '%s'", obs_source_get_name(itemSource), obs_source_get_id(itemSource), obs_source_get_name(sceneSource));
	}
}
...
void SourceTree::UpdateNoSourcesMessage()
{
	...
	QString file = lightTheme ? ":res/images/no_sources.svg" : darkPath.c_str();
