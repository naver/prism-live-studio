...
CaptionStream::CaptionStream(DWORD samplerate_, mssapi_captions *handler_) : handler(handler_), samplerate(samplerate_), event(CreateEvent(nullptr, false, false, nullptr))
{
...
STDMETHODIMP CaptionStream::CopyTo(IStream *stream, ULARGE_INTEGER bytes, ULARGE_INTEGER *read_bytes, ULARGE_INTEGER *written_bytes)
{
	HRESULT hr;
