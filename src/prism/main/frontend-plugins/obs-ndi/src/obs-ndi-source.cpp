...
obs_properties_t *ndi_source_getproperties(void *data)
{
	...
	obs_property_t *source_list = obs_properties_add_list(props, PROP_SOURCE, obs_module_text("NDIPlugin.SourceProps.SourceName"), OBS_COMBO_TYPE_EDITABLE, OBS_COMBO_FORMAT_STRING);
	obs_property_set_placeholder(source_list, obs_module_text("NDIPlugin.SourceProps.SourceName.NotFound"));
	obs_property_set_list_readonly(source_list, true);
	...
	obs_property_t *yuv_spaces = obs_properties_add_list(props, PROP_YUV_COLORSPACE, obs_module_text("NDIPlugin.SourceProps.ColorSpace"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
...
#if defined(_WIN32)
		ShellExecuteA(NULL, "open", "http://ndi.newtek.com", NULL, NULL, SW_SHOWNORMAL);
...
void *ndi_source_poll_audio_video(void *data)
{
	...
	while (s->running) {
		...
		if (frame_received == NDIlib_frame_type_video) {
			...
			video_format_get_parameters(s->yuv_colorspace, s->yuv_range, obs_video_frame.color_matrix, obs_video_frame.color_range_min, obs_video_frame.color_range_max);
		...
		if (ndiLib->recv_get_no_connections(s->ndi_receiver) == 0) {
			obs_source_output_audio(s->source, nullptr);
...
bool ndi_check_is_all_defaults(obs_data_t *settings)
{
	const char *ndi_name = obs_data_get_string(settings, PROP_SOURCE);
	if (ndi_name && ndi_name[0]) {
		return false;
	}

	if (obs_data_get_int(settings, PROP_BANDWIDTH) != PROP_BW_HIGHEST) {
		return false;
	}

	if (obs_data_get_int(settings, PROP_SYNC) != PROP_SYNC_NDI_SOURCE_TIMECODE) {
		return false;
	}

	if (obs_data_get_int(settings, PROP_YUV_RANGE) != PROP_YUV_RANGE_PARTIAL) {
		return false;
	}

	if (obs_data_get_int(settings, PROP_YUV_COLORSPACE) != PROP_YUV_SPACE_BT709) {
		return false;
	}
	...
	if (obs_data_get_bool(settings, PROP_HW_ACCEL)) {
		return false;
	}
...
void ndi_source_update(void *data, obs_data_t *settings)
{
	...
	bool is_all_defaults = ndi_check_is_all_defaults(settings);
	if (is_all_defaults && s->is_all_defaults) {
		return;
	}

	s->is_all_defaults = is_all_defaults;
...
struct obs_source_info create_ndi_source_info()
{
	...
	ndi_source_info.icon_type = OBS_ICON_TYPE_NDI;
