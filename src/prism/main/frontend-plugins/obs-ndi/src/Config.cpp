...
Config::Config() : OutputEnabled(false), OutputName("OBS"), PreviewOutputEnabled(false), PreviewOutputName("OBS Preview")
{
	...
	if (obs_config) {
		...
		config_set_default_string(obs_config, SECTION_NAME, PARAM_PREVIEW_OUTPUT_NAME, PreviewOutputName.toUtf8().constData());
	}
}
...
void Config::Save()
{
	...
	if (obs_config) {
		...
		config_set_string(obs_config, SECTION_NAME, PARAM_PREVIEW_OUTPUT_NAME, PreviewOutputName.toUtf8().constData());
