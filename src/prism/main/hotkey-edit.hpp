...
class PLSHotkeyLabel : public QLabel {
	Q_OBJECT

public:
	QPointer<PLSHotkeyLabel> pairPartner;
	QPointer<PLSHotkeyWidget> widget;
...
class PLSHotkeyEdit : public QLineEdit {
	Q_OBJECT;

public:
	explicit PLSHotkeyEdit(obs_key_combination_t original, QWidget *parent = nullptr) : QLineEdit(parent), original(original)
	{
#ifdef __APPLE__
...
class PLSHotkeyWidget : public QWidget {
	Q_OBJECT;

public:
	explicit PLSHotkeyWidget(obs_hotkey_id id, std::string name, const std::vector<obs_key_combination_t> &combos = {}, QWidget *parent = nullptr)
		: QWidget(parent), id(id), name(name), bindingsChanged(obs_get_signal_handler(), "hotkey_bindings_changed", &PLSHotkeyWidget::BindingsChanged, this)
	{
...
private:
	...
	std::vector<QPointer<PLSHotkeyEdit>> edits;
	OBSSignal bindingsChanged;
	bool ignoreChangedBindings = false;
	bool clearBtnEnabled = true;
...
signals:
	void KeyChanged();
	void clearButtonClicked(QPushButton *button);
};
