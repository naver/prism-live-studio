...
bool PLSBasic::AddProfile(bool create_new, const char *title, const char *text, const char *init_text, bool rename, bool addDefault)
{
	...
	if (!addDefault && !GetProfileName(this, newName, newDir, title, text, init_text))
		return false;
	...
	blog(LOG_INFO, "Created profile '%s' (%s, %s)", newName.c_str(), create_new ? "clean" : "duplicate", newDir.c_str());
	blog(LOG_INFO, "------------------------------------------------");
...
void PLSBasic::RefreshProfiles()
{
	...
	auto addProfile = [&](const char *name, const char *path) {
		...
		connect(action, &QAction::triggered, this, &PLSBasic::ChangeProfile);
...
void PLSBasic::on_actionImportProfile_triggered()
{
	...
	QString dir = QFileDialog::getExistingDirectory(this, QTStr("Basic.MainMenu.Profile.Import"), home, QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
	...
	if (!dir.isEmpty() && !dir.isNull()) {
		...
		if (!folder.exists()) {
			...
			PLSMessageBox::warning(this, QTStr("Basic.MainMenu.Profile.Import"), QTStr("Basic.MainMenu.Profile.Exists"));
		}
	}
}
...
void PLSBasic::on_actionExportProfile_triggered()
{
	...
	QString dir = QFileDialog::getExistingDirectory(this, QTStr("Basic.MainMenu.Profile.Export"), home, QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
