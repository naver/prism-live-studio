...
#define ThisIsValid [this](QObject *object) { return naverShoppingLIVEProductsInstances.contains(object); }
#define MAX_SEARCH_KEYWORDS_COUNT 15
#define MAX_SELECTED_PRODUCT_COUNT 30
#define TAB_MIN 0
#define TAB_MAX 3
#define RECENT_TAB 0
#define STORE_TAB 1
...
static QList<QObject *> naverShoppingLIVEProductsInstances;
...
{
	QPushButton *buttons[] = {others...};
	for (int i = 0, count = sizeof...(PushButton); i < count; ++i) {
...
static void listenVerticalScrollBar(QScrollBar *&scrollBar, QScrollArea *scrollArea, QLayout *layout, QObject *listener)
{
	if (scrollBar = scrollArea->verticalScrollBar(); scrollBar) {
		PLSDpiHelper::setDynamicContentsMargins(layout, true);
		scrollBar->installEventFilter(listener);
	}
}

static void processScrollContentMarginRight(QScrollBar *scrollBar, QLayout *layout, const QMargins &margins, bool show)
{
	if (show) {
...
static void processScrollContentMarginRight(PLSNaverShoppingLIVEProductDialogView *dlg, QScrollBar *scrollBar, QLayout *layout, bool show, bool isProduct)
{
	processScrollContentMarginRight(scrollBar, layout, PLSDpiHelper::calculate(dlg, isProduct ? QMargins(24, 0, 29, 20) : QMargins(29, 0, 29, 20)), show);
}

static void processScrollContentMarginRight(PLSNaverShoppingLIVEProductDialogView *dlg, QScrollBar *scrollBar, QLayout *layout, bool isProduct)
{
	if (scrollBar) {
		processScrollContentMarginRight(scrollBar, layout, PLSDpiHelper::calculate(dlg, isProduct ? QMargins(24, 0, 29, 20) : QMargins(29, 0, 29, 20)), scrollBar->isVisible());
	}
}

void pls_login_async(std::function<void(bool ok, const QJsonObject &result)> &&callback, const QString &platformName, QWidget *parent, PLSLoginInfo::UseFor useFor);

PLSNaverShoppingLIVEProductDialogView::PLSNaverShoppingLIVEProductDialogView(PLSPlatformNaverShoppingLIVE *platform_, const QList<Product> &selectedProducts_, bool isLiving_, QWidget *parent,
									     PLSDpiHelper dpiHelper)
	: PLSDialogView(parent, dpiHelper), ui(new Ui::PLSNaverShoppingLIVEProductDialogView), platform(platform_), selectedProducts(selectedProducts_), isLiving(isLiving_)
{
	naverShoppingLIVEProductsInstances.append(this);
	setResizeEnabled(false);

	dpiHelper.setCss(this, {PLSCssIndex::PLSNaverShoppingLIVEProductDialogView});
	dpiHelper.setFixedSize(this, {580, 650});
	...
	if (int latestUseTab = manager->getLatestUseTab(); (latestUseTab >= TAB_MIN) && (latestUseTab < TAB_MAX)) {
		switch (latestUseTab) {
		case RECENT_TAB:
			//if (manager->hasRecentProductNos()) {
	...
	notifyDpiChangedBefore([this](bool firstShow) {
		if (!firstShow) {
			PLSNaverShoppingLIVEProductItemView::resetIconSize(recentProductItemViews);
			PLSNaverShoppingLIVEProductItemView::resetIconSize(storeProductItemViews);
			PLSNaverShoppingLIVEProductItemView::resetIconSize(searchProductItemViews);
		}
	});
...
{
	naverShoppingLIVEProductsInstances.removeOne(this);
...
void PLSNaverShoppingLIVEProductDialogView::initRecentProducts(int currentPage)
{
	...
	if (isFirstPage && !PLSNetworkMonitor::Instance()->IsInternetAvailable()) {
	...
	} else if (PLSNetworkMonitor::Instance()->IsInternetAvailable()) {
...
void PLSNaverShoppingLIVEProductDialogView::initSmartStores()
{
	...
	connect(manager, &PLSNaverShoppingLIVEDataManager::smartStoreInfoUpdated, this, [this, manager](bool ok, const QString &storeId, const QString &storeName) {
		if (ok) {
				...
				if (!products.isEmpty()) {
					...
					for (const auto &product : products) {
						PLSNaverShoppingLIVEProductItemView *view = PLSNaverShoppingLIVEProductItemView::alloc(
							...
							[this](PLSNaverShoppingLIVEProductItemView *itemView, qint64 productNo) {
								onProductItemAddRemoveButtonClicked(&storeProductItemViews, itemView, productNo);
							},
							platform, product);
...
void PLSNaverShoppingLIVEProductDialogView::searchSearch(const QString &text, int currentPage)
{
	...
	if (text.startsWith("http://") || text.startsWith("https://")) {
		PLSNaverShoppingLIVEAPI::productSearchByUrl(
			platform, text,
			[this, searchIndex = getSearchIndex(searchSearchIndex)](bool ok, bool hasProduct, const Product &product) {
				...
				if (ok) {
					if (hasProduct) {
						...
						view = PLSNaverShoppingLIVEProductItemView::alloc(
							...
							[this](PLSNaverShoppingLIVEProductItemView *itemView, qint64 productNo) {
								onProductItemAddRemoveButtonClicked(&searchProductItemViews, itemView, productNo);
							},
							platform, product);
...
{
	for (const auto &selectedProduct : selectedProducts) {
		if (selectedProduct.productNo == productNo) {
			return true;
		}
	}
	return false;
...
bool PLSNaverShoppingLIVEProductDialogView::findItemView(PLSNaverShoppingLIVEProductItemView *&itemView, const QList<PLSNaverShoppingLIVEProductItemView *> &itemViews, qint64 productNo)
{
...
void PLSNaverShoppingLIVEProductDialogView::syncItemViewSelectedState(QList<PLSNaverShoppingLIVEProductItemView *> *current, const QList<PLSNaverShoppingLIVEProductItemView *> &itemViews,
								      qint64 productNo, bool selected)
{
...
void PLSNaverShoppingLIVEProductDialogView::syncItemViewSelectedState(QList<PLSNaverShoppingLIVEProductItemView *> *current, qint64 productNo, bool selected)
{
...
{
	ui->countTipLabel->setVisible(selectedProducts.count() > 0);
...
void PLSNaverShoppingLIVEProductDialogView::updateSmartStores(bool apiFailed)
{
	...
	// ui->storeChangeStoreButtonContainer->setVisible(platform->getAccountType() == NaverShoppingAccountType::NaverShoppingSelective);
...
bool PLSNaverShoppingLIVEProductDialogView::asyncProcessAndCheckDone()
{
	// QApplication::processEvents(QEventLoop::ExcludeUserInputEvents);
	return doneFlag;
}

QLayout *PLSNaverShoppingLIVEProductDialogView::vscrollBarToLayout(QObject *vscrollBar) const
{
...
void PLSNaverShoppingLIVEProductDialogView::onProductItemAddRemoveButtonClicked(QList<PLSNaverShoppingLIVEProductItemView *> *itemViews, PLSNaverShoppingLIVEProductItemView *itemView,
										qint64 productNo)
{
	PLS_UI_STEP(MODULE_NAVER_SHOPPING_LIVE_PRODUCT_MANAGER, "Add Or Remove Product Button", ACTION_CLICK);

	//PLSNaverShoppingLIVEDataManager *manager = PLSNaverShoppingLIVEDataManager::instance();
	if (isSelectedProductNo(productNo)) {
		...
		if (!details.isMinorPurchasable) {
			PLSAlertView::information(this, tr("Alert.Title"), tr("navershopping.liveinfo.age.restrict.product"), PLSAlertView::Button::Ok, PLSAlertView::Button::Ok);
			return;
		}
		...
		//if (manager->addRecentProductNo(QString::number(productNo))) {
		//	onAddRecentProduct(itemView, productNo);
		//}
	} else {
		itemView->setSelected(false);
		syncItemViewSelectedState(itemViews, productNo, false);
...
void PLSNaverShoppingLIVEProductDialogView::onRemoveButtonClicked(PLSNaverShoppingLIVESearchKeywordItemView *itemView)
{
...
void PLSNaverShoppingLIVEProductDialogView::onSearchButtonClicked(PLSNaverShoppingLIVESearchKeywordItemView *itemView)
{
...
void PLSNaverShoppingLIVEProductDialogView::onAddRecentProduct(PLSNaverShoppingLIVEProductItemView *itemView, qint64)
{
...
bool PLSNaverShoppingLIVEProductDialogView::eventFilter(QObject *watched, QEvent *event)
{
	if (watched == ui->storeSearchBarLineEdit || watched == ui->searchSearchBarLineEdit) {
		...
		switch (event->type()) {
		case QEvent::Resize: {
			QString storeName = getCurrentSmartStoreName();
			if (static_cast<QResizeEvent *>(event)->size().width() == 0) {
				...
				QString text = ui->storeNameLabel->fontMetrics().elidedText(storeName, Qt::ElideRight, width);
