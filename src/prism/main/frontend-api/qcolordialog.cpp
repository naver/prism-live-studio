...
public:
	enum SetColorMode { ShowColor = 0x1, SelectColor = 0x2, SetColorAll = ShowColor | SelectColor };

	QColorDialogPrivate()
		: options(QColorDialogOptions::create())
#ifdef Q_OS_WIN32
		  ,
		  updateTimer(0)
#endif
	...
	QPlatformColorDialogHelper *platformColorDialogHelper() const { return static_cast<QPlatformColorDialogHelper *>(platformHelper()); }
...
public:
	...
	inline int cellWidth() const { return PLSDpiHelper::calculate(const_cast<QWellArray *>(this), cellw); }

	inline int cellHeight() const { return PLSDpiHelper::calculate(const_cast<QWellArray *>(this), cellh); }

	inline int rowAt(int y) const { return y / cellHeight(); }

	inline int columnAt(int x) const
	{
		if (isRightToLeft())
			return ncols - (x / cellWidth()) - 1;
		return x / cellWidth();
	}

	inline int rowY(int row) const { return cellHeight() * row; }

	inline int columnX(int column) const
	{
		if (isRightToLeft())
			return cellWidth() * (ncols - column - 1);
	...
	inline QRect cellRect() const { return QRect(0, 0, cellWidth(), cellHeight()); }

	inline QSize gridSize() const { return QSize(ncols * cellWidth(), nrows * cellHeight()); }

	QRect cellGeometry(int row, int column)
	{
		QRect r;
		if (row >= 0 && row < nrows && column >= 0 && column < ncols)
			r.setRect(columnX(column), rowY(row), cellWidth(), cellHeight());
		return r;
	}
...
QWellArray::QWellArray(int rows, int cols, QWidget *parent) : QWidget(parent), nrows(rows), ncols(cols)
{
...
{
	ensurePolished();
	return gridSize().boundedTo(PLSDpiHelper::calculate(const_cast<QWellArray *>(this), QSize(640, 480)));
}

void QWellArray::paintCell(QPainter *p, int row, int col, const QRect &rect)
{
	double dpi = PLSDpiHelper::getDpi(this);
	int b = PLSDpiHelper::calculate(dpi, 3); //margin

	const QPalette &g = palette();
	QStyleOptionFrame opt;
	int dfw = style()->pixelMetric(QStyle::PM_DefaultFrameWidth);
	opt.lineWidth = dfw;
	opt.midLineWidth = PLSDpiHelper::calculate(dpi, 1);
...
public:
	QColorWell(QWidget *parent, int r, int c, const QRgb *vals) : QWellArray(r, c, parent), values(vals), mousePressed(false), oldCurrent(-1, -1)
	{
...
{
	double dpi = PLSDpiHelper::getDpi(this);
	int coffDpi = PLSDpiHelper::calculate(dpi, coff);
	int d = height() - 2 * coffDpi - PLSDpiHelper::calculate(dpi, 1);
	return 255 - (y - coffDpi) * 255 / d;
}
...
int QColorLuminancePicker::val2y(int v)
{
	...
	int coffDpi = PLSDpiHelper::calculate(dpi, coff);
	int d = height() - 2 * coffDpi - PLSDpiHelper::calculate(dpi, 1);
	return coffDpi + (255 - v) * d / 255;
}

QColorLuminancePicker::QColorLuminancePicker(QWidget *parent) : QWidget(parent)
{
	hue = 100;
	val = 100;
	sat = 100;
...
{
	double dpi = PLSDpiHelper::getDpi(this);
	int fiveDpi = PLSDpiHelper::calculate(dpi, 5);
	int oneDpi = PLSDpiHelper::calculate(dpi, 1);
	int twoDpi = PLSDpiHelper::calculate(dpi, 2);
	int foffDpi = PLSDpiHelper::calculate(dpi, foff);
	int coffDpi = PLSDpiHelper::calculate(dpi, coff);

	int w = width() - fiveDpi;

	QRect r(0, foffDpi, w, height() - 2 * foffDpi);
	int wi = r.width() - twoDpi;
	int hi = r.height() - twoDpi;
	if (!pix || pix->height() != hi || pix->width() != wi) {
		...
		for (y = 0; y < hi; y++) {
			uint *end = pixel + wi;
			std::fill(pixel, end, QColor::fromHsv(hue, sat, y2val(y + coffDpi)).rgb());
			pixel = end;
		}
		pix = new QPixmap(QPixmap::fromImage(img));
	}
	QPainter p(this);
	p.drawPixmap(oneDpi, coffDpi, *pix);
	const QPalette &g = palette();
	...
	QPolygon a;
	int y = val2y(val);
	a.setPoints(3, w, y, w + fiveDpi, y + fiveDpi, w + fiveDpi, y - fiveDpi);
	p.eraseRect(w, 0, fiveDpi, height());
	p.drawPolygon(a);
}
...
QColorPicker::QColorPicker(QWidget *parent) : QFrame(parent), crossVisible(true)
{
	hue = 0;
	sat = 0;
	setCol(150, 255);
...
class QColorShower : public QWidget {
	Q_OBJECT
public:
	QColorShower(QColorDialog *parent, PLSDpiHelper dpiHelper);

	//things that don't emit signals
	void setHsv(int h, int s, int v);

	int currentAlpha() const { return (colorDialog->options() & QColorDialog::ShowAlphaChannel) ? alphaEd->value() : 255; }
	void setCurrentAlpha(int a)
	{
		alphaEd->setValue(a);
		rgbEd();
...
QColorShower::QColorShower(QColorDialog *parent, PLSDpiHelper dpiHelper) : QWidget(parent)
{
	colorDialog = parent;
...
#endif
	dpiHelper.setMinimumWidth(lab, 60);
...
void QColorDialogPrivate::initWidgets()
{
	...
	if (!smallDisplay) {
		...
		screenColorPickerButton->setObjectName("screenColorPickerButton");
		...
		addCusBt->setObjectName("addCusBt");
	...
	lp->hide();
#else
	dpiHelper.setFixedWidth(lp, 20);
	pickLay->addSpacing(10);
	pickLay->addWidget(lp);
	pickLay->addSpacing(30);
#endif
	...
	cs = new QColorShower(q, dpiHelper);
	pickLay->setMargin(cs->gl->margin());
	QObject::connect(cs, SIGNAL(newCol(QRgb)), q, SLOT(_q_newColorTypedIn(QRgb)));
	QObject::connect(cs, SIGNAL(currentColorChanged(QColor)), q, SIGNAL(currentColorChanged(QColor)));
#if defined(QT_SMALL_COLORDIALOG)
	topLay->addWidget(cs);
#else
...
bool QColorDialogPrivate::canBeNativeDialog() const
{
	...
	if (nativeDialogInUse)
		return true;
	if (QCoreApplication::testAttribute(Qt::AA_DontUseNativeDialogs) || q->testAttribute(Qt::WA_DontShowOnScreen) || (options->options() & QColorDialog::DontUseNativeDialog)) {
		return false;
	}
...
static const Qt::WindowFlags DefaultWindowFlags = Qt::Dialog | Qt::WindowTitleHint | Qt::WindowSystemMenuHint | Qt::WindowCloseButtonHint;
...
*/
QColorDialog::QColorDialog(QWidget *parent) : QColorDialog(QColor(Qt::white), parent) {}
...
/*!
    ...
    \a initial color.
*/
QColorDialog::QColorDialog(const QColor &initial, QWidget *parent) : QDialog(*new QColorDialogPrivate, parent, DefaultWindowFlags)
{
	Q_D(QColorDialog);
	d->init(initial);
}
...
{
	const QColor color = getColor(QColor::fromRgba(initial), parent, QString(), ShowAlphaChannel);
