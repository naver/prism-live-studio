...
class PLSRemux : public PLSDialogView {
	Q_OBJECT
...
public:
	explicit PLSRemux(const char *recPath, QWidget *parent = nullptr, bool autoRemux = false);
	virtual ~PLSRemux() override;
...
class RemuxQueueModel : public QAbstractTableModel {
	...
	friend class PLSRemux;

public:
	explicit RemuxQueueModel(QObject *parent = 0) : QAbstractTableModel(parent), isProcessing(false) {}
...
public:
	explicit RemuxEntryPathItemDelegate(bool isOutput, const QString &defaultPath);
