...
PLSUpdateView::PLSUpdateView(bool manualUpdate, bool isForceUpdate, const QString &version, const QString &fileUrl, const QString &updateInfoUrl, QWidget *parent)
	: PLSDialogView(parent), ui(new Ui::PLSUpdateView), m_manualUpdate(manualUpdate), m_isForceUpdate(isForceUpdate), m_version(version), m_fileUrl(fileUrl), m_updateInfoUrl(updateInfoUrl)
{
	initUI();
}
...
void PLSUpdateView::initUI()
{
	...
	if (m_isForceUpdate) {
		...
		ui->nowUpdateBtn->setText(tr("update.bottom.confirm.button.text"));
	}
