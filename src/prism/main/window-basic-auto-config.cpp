...
static OBSData OpenServiceSettings(std::string &type)
{
	...
	OBSData data = obs_data_create_from_json_file_safe(serviceJsonPath, "bak");
...
AutoConfigVideoPage::AutoConfigVideoPage(QWidget *parent) : QWizardPage(parent), ui(new Ui_AutoConfigVideoPage)
{
	...
	QString fpsStr = (ovi.fps_den > 1) ? QString::number(fpsVal, 'f', 2) : QString::number(fpsVal, 'g', 2);
	...
	for (int i = 0; i < screens.size(); i++) {
		...
		QString str = QTStr(RES_USE_DISPLAY).arg(QString::number(i + 1), QString::number(as.width()), QString::number(as.height()));
...
AutoConfigStreamPage::AutoConfigStreamPage(QWidget *parent) : QWizardPage(parent), ui(new Ui_AutoConfigStreamPage)
{
	...
	connect(ui->service, SIGNAL(currentIndexChanged(int)), this, SLOT(ServiceChanged()));
	connect(ui->customServer, SIGNAL(textChanged(const QString &)), this, SLOT(ServiceChanged()));
	connect(ui->doBandwidthTest, SIGNAL(toggled(bool)), this, SLOT(ServiceChanged()));
	...
	connect(ui->key, SIGNAL(textChanged(const QString &)), this, SLOT(UpdateCompleted()));
	connect(ui->regionUS, SIGNAL(toggled(bool)), this, SLOT(UpdateCompleted()));
	connect(ui->regionEU, SIGNAL(toggled(bool)), this, SLOT(UpdateCompleted()));
	connect(ui->regionAsia, SIGNAL(toggled(bool)), this, SLOT(UpdateCompleted()));
	connect(ui->regionOther, SIGNAL(toggled(bool)), this, SLOT(UpdateCompleted()));
}
...
bool AutoConfigStreamPage::validatePage()
{
	...
	OBSService service = obs_service_create(serverType, "temp_service", service_settings, nullptr);
...
#define WARNING_TEXT(x) QTStr("Basic.AutoConfig.StreamPage.StreamWarning." x)
		PLSAlertView::Button button = PLSMessageBox::question(this, WARNING_TEXT("Title"), WARNING_TEXT("Text"));
#undef WARNING_TEXT
...
void AutoConfigStreamPage::ServiceChanged()
{
	...
	if (cef) {
		if (lastService != service.c_str()) {
			...
			int page = can_auth ? (int)Section::Connect : (int)Section::StreamKey;
...
void AutoConfigStreamPage::UpdateKeyLink()
{
	...
	if (serviceName == "Twitch") {
		...
		text += " <a href=\"https://";
		text += "restream.io/settings/streaming-setup?from=PLS";
		text += "\">";
		text += QTStr("Basic.AutoConfig.StreamPage.StreamKey.LinkToSite");
		text += "</a>";
...
void AutoConfigStreamPage::UpdateCompleted()
{
	if (ui->stackedWidget->currentIndex() == (int)Section::Connect || (ui->key->text().isEmpty() && !auth)) {
		...
		if (custom) {
			...
			ready = !wiz->testRegions || ui->regionUS->isChecked() || ui->regionEU->isChecked() || ui->regionAsia->isChecked() || ui->regionOther->isChecked();
		}
	}
...
AutoConfig::AutoConfig(QWidget *parent) : PLSWizardView(parent)
{
