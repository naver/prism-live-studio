...
PLSBasicInteraction::PLSBasicInteraction(QWidget *parent, OBSSource source_)
	: PLSDialogView(parent),
	  main(qobject_cast<PLSBasic *>(parent)),
	  ui(new Ui::PLSBasicInteraction),
	  source(source_),
	  removedSignal(obs_source_get_signal_handler(source), "remove", PLSBasicInteraction::SourceRemoved, this),
	  renamedSignal(obs_source_get_signal_handler(source), "rename", PLSBasicInteraction::SourceRenamed, this),
	  eventFilter(BuildEventFilter())
{
	...
	auto addDrawCallback = [this]() { obs_display_add_draw_callback(ui->preview->GetDisplay(), PLSBasicInteraction::DrawPreview, this); };

	connect(ui->preview, &PLSQTDisplay::DisplayCreated, addDrawCallback);
}

PLSBasicInteraction::~PLSBasicInteraction()
{
...
PLSEventFilter *PLSBasicInteraction::BuildEventFilter()
{
	return new PLSEventFilter([this](QObject *obj, QEvent *event) {
...
void PLSBasicInteraction::SourceRemoved(void *data, calldata_t *params)
{
	QMetaObject::invokeMethod(static_cast<PLSBasicInteraction *>(data), "close");

	UNUSED_PARAMETER(params);
}

void PLSBasicInteraction::SourceRenamed(void *data, calldata_t *params)
{
...
void PLSBasicInteraction::DrawPreview(void *data, uint32_t cx, uint32_t cy)
{
	PLSBasicInteraction *window = static_cast<PLSBasicInteraction *>(data);
...
void PLSBasicInteraction::closeEvent(QCloseEvent *event)
{
	...
	obs_display_remove_draw_callback(ui->preview->GetDisplay(), PLSBasicInteraction::DrawPreview, this);
}
...
bool PLSBasicInteraction::GetSourceRelativeXY(int mouseX, int mouseY, int &relX, int &relY)
{
...
bool PLSBasicInteraction::HandleMouseClickEvent(QMouseEvent *event)
{
	...
	bool insideSource = GetSourceRelativeXY(event->x(), event->y(), mouseEvent.x, mouseEvent.y);
...
bool PLSBasicInteraction::HandleMouseMoveEvent(QMouseEvent *event)
{
	struct obs_mouse_event mouseEvent = {};
	...
	if (!mouseLeave) {
		mouseEvent.modifiers = TranslateQtMouseEventModifiers(event);
		mouseLeave = !GetSourceRelativeXY(event->x(), event->y(), mouseEvent.x, mouseEvent.y);
	}
...
bool PLSBasicInteraction::HandleMouseWheelEvent(QWheelEvent *event)
{
	struct obs_mouse_event mouseEvent = {};
	...
	if (GetSourceRelativeXY(event->x(), event->y(), mouseEvent.x, mouseEvent.y))
		obs_source_send_mouse_wheel(source, &mouseEvent, xDelta, yDelta);

	return true;
}

bool PLSBasicInteraction::HandleFocusEvent(QFocusEvent *event)
{
...
bool PLSBasicInteraction::HandleKeyEvent(QKeyEvent *event)
{
