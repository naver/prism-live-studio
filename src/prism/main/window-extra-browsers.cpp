...
void ExtraBrowsersModel::Reset()
{
	...
	for (int i = 0; i < main->extraBrowserDocks.size(); i++) {
		BrowserDock *dock = reinterpret_cast<BrowserDock *>(main->extraBrowserDocks[i].data());
...
void ExtraBrowsersModel::UpdateItem(Item &item)
{
	...
	PLSBasic *main = PLSBasic::Get();
	BrowserDock *dock = reinterpret_cast<BrowserDock *>(main->extraBrowserDocks[idx].data());
...
QWidget *ExtraBrowsersDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &, const QModelIndex &index) const
{
...
PLSExtraBrowsers::PLSExtraBrowsers(QWidget *parent) : PLSDialogView(parent), ui(new Ui::PLSExtraBrowsers)
{
	...
	ui->table->horizontalHeader()->resizeSection((int)Column::Delete, 41);
	ui->table->verticalHeader()->setDefaultSectionSize(41);
	ui->table->verticalHeader()->setMinimumSectionSize(41);
	ui->table->setEditTriggers(QAbstractItemView::EditTrigger::CurrentChanged);
	connect(ui->close, &QPushButton::clicked, this, &PLSExtraBrowsers::close);
}

PLSExtraBrowsers::~PLSExtraBrowsers()
{
	delete ui;
}

void PLSExtraBrowsers::closeEvent(QCloseEvent *event)
{
	QDialog::closeEvent(event);
	model->Apply();
}

void PLSExtraBrowsers::on_apply_clicked()
{
	model->Apply();
}
...
void PLSBasic::ClearExtraBrowserDocks()
{
...
void PLSBasic::LoadExtraBrowserDocks()
{
...
void PLSBasic::SaveExtraBrowserDocks()
{
	Json::array array;
