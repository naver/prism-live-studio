...
void PLSBasicTransform::HookWidget(QWidget *widget, const char *signal, const char *slot)
{
...
PLSBasicTransform::PLSBasicTransform(PLSBasic *parent, PLSDpiHelper dpiHelper) : PLSDialogView(parent, dpiHelper), ui(new Ui::PLSBasicTransform), main(parent)
{
	...
	connect(ui->buttonBox, &PLSDialogButtonBox::rejected, this, &PLSDialogView::reject);
	...
	channelChangedSignal.Connect(obs_get_signal_handler(), "channel_change", PLSChannelChanged, this);
}

void PLSBasicTransform::SetScene(OBSScene scene)
{
...
void PLSBasicTransform::SetItem(OBSSceneItem newItem)
{
	QMetaObject::invokeMethod(this, "SetItemQt", Q_ARG(OBSSceneItem, OBSSceneItem(newItem)));
}

void PLSBasicTransform::SetItemQt(OBSSceneItem newItem)
{
	item = newItem;
	if (item)
		RefreshControls();

	content()->setEnabled(!!item);
}

void PLSBasicTransform::PLSChannelChanged(void *param, calldata_t *data)
{
	PLSBasicTransform *window = reinterpret_cast<PLSBasicTransform *>(param);
...
void PLSBasicTransform::OBSSceneItemTransform(void *param, calldata_t *data)
{
	PLSBasicTransform *window = reinterpret_cast<PLSBasicTransform *>(param);
...
void PLSBasicTransform::OBSSceneItemRemoved(void *param, calldata_t *data)
{
	PLSBasicTransform *window = reinterpret_cast<PLSBasicTransform *>(param);
...
void PLSBasicTransform::OBSSceneItemSelect(void *param, calldata_t *data)
{
	PLSBasicTransform *window = reinterpret_cast<PLSBasicTransform *>(param);
	OBSSceneItem item = (obs_sceneitem_t *)calldata_ptr(data, "item");
...
void PLSBasicTransform::OBSSceneItemDeselect(void *param, calldata_t *data)
{
	PLSBasicTransform *window = reinterpret_cast<PLSBasicTransform *>(param);
	OBSScene scene = (obs_scene_t *)calldata_ptr(data, "scene");
	OBSSceneItem item = (obs_sceneitem_t *)calldata_ptr(data, "item");
...
static const uint32_t listToAlign[] = {OBS_ALIGN_TOP | OBS_ALIGN_LEFT,    OBS_ALIGN_TOP,    OBS_ALIGN_TOP | OBS_ALIGN_RIGHT,   OBS_ALIGN_LEFT, OBS_ALIGN_CENTER, OBS_ALIGN_RIGHT,
...
void PLSBasicTransform::RefreshControls()
{
	if (!item)
		return;
...
void PLSBasicTransform::OnBoundsType(int index)
{
	if (index == -1)
		return;
...
void PLSBasicTransform::OnControlChanged()
{
	if (ignoreItemChange)
		return;
...
void PLSBasicTransform::OnCropChanged()
{
	if (ignoreItemChange)
		return;
...
void PLSBasicTransform::on_resetButton_clicked()
{
