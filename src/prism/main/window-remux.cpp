...
RemuxEntryPathItemDelegate::RemuxEntryPathItemDelegate(bool isOutput, const QString &defaultPath) : QStyledItemDelegate(), isOutput(isOutput), defaultPath(defaultPath) {}

QWidget *RemuxEntryPathItemDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem & /* option */, const QModelIndex &index) const
{
	RemuxEntryState state = index.model()->index(index.row(), RemuxEntryColumn::State).data(RemuxEntryRole::EntryStateRole).value<RemuxEntryState>();
	if (state == RemuxEntryState::Pending || state == RemuxEntryState::InProgress) {
		...
		QSizePolicy buttonSizePolicy(QSizePolicy::Policy::Minimum, QSizePolicy::Policy::Expanding, QSizePolicy::ControlType::PushButton);

		QWidget *container = new QWidget(parent);

		auto browseCallback = [this, container]() { const_cast<RemuxEntryPathItemDelegate *>(this)->handleBrowse(container); };

		auto clearCallback = [this, container]() { const_cast<RemuxEntryPathItemDelegate *>(this)->handleClear(container); };
		...
		text->setProperty("For", QStringLiteral("SelectPath"));
		...
		browseButton->setProperty("For", QStringLiteral("SelectPath"));
		...
		if (!isOutput && state != RemuxEntryState::Empty) {
			...
			clearButton->setProperty("For", QStringLiteral("SelectPathClear"));
...
void RemuxEntryPathItemDelegate::setModelData(QWidget *editor, QAbstractItemModel *model, const QModelIndex &index) const
{
...
void RemuxEntryPathItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
	RemuxEntryState state = index.model()->index(index.row(), RemuxEntryColumn::State).data(RemuxEntryRole::EntryStateRole).value<RemuxEntryState>();
	...
	if (isOutput) {
		if (state != Ready) {
			QColor background = localOption.palette.color(QPalette::ColorGroup::Disabled, QPalette::ColorRole::Background);
...
void RemuxEntryPathItemDelegate::handleBrowse(QWidget *container)
{
	...
	bool isSet = false;
	if (isOutput) {
		QString newPath = QFileDialog::getSaveFileName(container, QTStr("Remux.SelectTarget"), currentPath, OutputPattern);
		...
		QStringList paths = QFileDialog::getOpenFileNames(container, QTStr("Remux.SelectRecording"), currentPath, QTStr("Remux.PLSRecording") + QString(" ") + InputPattern);
...
void RemuxQueueModel::checkInputPath(int row)
{
	...
	if (entry.sourcePath.isEmpty()) {
		...
		if (entry.state == RemuxEntryState::Ready)
			entry.targetPath = fileInfo.path() + QDir::separator() + fileInfo.completeBaseName() + ".mp4";
	}
...
PLSRemux::PLSRemux(const char *path, QWidget *parent, bool autoRemux_)
	: PLSDialogView(parent), queueModel(new RemuxQueueModel), worker(new RemuxWorker()), ui(new Ui::PLSRemux), recPath(path), autoRemux(autoRemux_)
{
	setAcceptDrops(true);
	...
	ui->tableView->setItemDelegateForColumn(RemuxEntryColumn::InputPath, new RemuxEntryPathItemDelegate(false, recPath));
	ui->tableView->setItemDelegateForColumn(RemuxEntryColumn::OutputPath, new RemuxEntryPathItemDelegate(true, recPath));
	...
	connect(ui->buttonBox->button(QDialogButtonBox::Ok), SIGNAL(clicked()), this, SLOT(beginRemux()));
	connect(ui->buttonBox->button(QDialogButtonBox::Reset), SIGNAL(clicked()), this, SLOT(clearFinished()));
	connect(ui->buttonBox->button(QDialogButtonBox::RestoreDefaults), SIGNAL(clicked()), this, SLOT(clearAll()));
	...
	connect(worker_, &RemuxWorker::updateProgress, this, &PLSRemux::updateProgress);
	connect(&remuxer, &QThread::finished, worker_, &QObject::deleteLater);
	connect(worker_, &RemuxWorker::remuxFinished, this, &PLSRemux::remuxFinished);
	connect(this, &PLSRemux::remux, worker_, &RemuxWorker::remux);
	...
	connect(queueModel_, SIGNAL(rowsInserted(const QModelIndex &, int, int)), this, SLOT(rowCountChanged(const QModelIndex &, int, int)));
	connect(queueModel_, SIGNAL(rowsRemoved(const QModelIndex &, int, int)), this, SLOT(rowCountChanged(const QModelIndex &, int, int)));

	QModelIndex index = queueModel->createIndex(0, 1);
	QMetaObject::invokeMethod(ui->tableView, "setCurrentIndex", Qt::QueuedConnection, Q_ARG(const QModelIndex &, index));
}

bool PLSRemux::stopRemux()
{
	if (!worker->isWorking)
		return true;
	...
	if (PLSAlertView::critical(nullptr, QTStr("Remux.ExitUnfinishedTitle"), QTStr("Remux.ExitUnfinished"), PLSAlertView::Button::Yes | PLSAlertView::Button::No, PLSAlertView::Button::No) ==
...
void PLSRemux::rowCountChanged(const QModelIndex &, int, int)
{
...
void PLSRemux::dropEvent(QDropEvent *ev)
{
	QStringList urlList;
	...
	for (QUrl url : ev->mimeData()->urls()) {
		...
		if (fileInfo.isDir()) {
			...
			QDirIterator dirIter(fileInfo.absoluteFilePath(), directoryFilter, QDir::Files, QDirIterator::Subdirectories);
	...
	if (urlList.empty()) {
		PLSAlertView::information(nullptr, QTStr("Remux.NoFilesAddedTitle"), QTStr("Remux.NoFilesAdded"), PLSAlertView::Button::Ok);
	} else if (!autoRemux) {
...
void PLSRemux::dragEnterEvent(QDragEnterEvent *ev)
{
	if (ev->mimeData()->hasUrls() && !worker->isWorking)
		ev->accept();
}

void PLSRemux::beginRemux()
{
	if (worker->isWorking) {
		stopRemux();
		return;
	...
	if (!overwriteFiles.empty()) {
		...
		if (PLSMessageBox::question(this, QTStr("Remux.FileExistsTitle"), message) != PLSAlertView::Button::Yes)
...
void PLSRemux::AutoRemux(QString inFile, QString outFile)
{
...
void PLSRemux::remuxNextEntry()
{
	...
	if (queueModel->beginNextEntry(inputPath, outputPath)) {
		...
		if (!autoRemux) {
			PLSMessageBox::information(this, QTStr("Remux.FinishedTitle"), queueModel->checkForErrors() ? QTStr("Remux.FinishedError") : QTStr("Remux.Finished"));
		}
...
void PLSRemux::closeEvent(QCloseEvent *event)
{
	if (!stopRemux())
		event->ignore();
	else
...
void PLSRemux::updateProgress(float percent)
{
	ui->progressBar->setValue(percent * 10);
}

void PLSRemux::remuxFinished(bool success)
{
	ui->buttonBox->button(QDialogButtonBox::Ok)->setEnabled(true);
...
void PLSRemux::clearFinished()
{
	queueModel->clearFinished();
}

void PLSRemux::clearAll()
{
	queueModel->clearAll();
}
