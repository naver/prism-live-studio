...
OAuthLogin::OAuthLogin(QWidget *parent, const std::string &url, bool token) : PLSDialogView(parent), get_token(token)
{
	if (!cef) {
		return;
	}
	...
	connect(close, &QAbstractButton::clicked, this, &PLSDialogView::reject);
	...
	QVBoxLayout *topLayout = new QVBoxLayout(this->content());
...
{
	if (cefWidget) {
		return PLSDialogView::exec();
	}

	return PLSDialogView::Rejected;
}
...
static inline std::string get_config_str(PLSBasic *main, const char *section, const char *name)
{
...
bool OAuth::GetToken(const char *url, const std::string &client_id, int scope_ver, const std::string &auth_code, bool retry)
try {
	std::string output;
	std::string error;
	std::string desc;
	...
	auto func = [&]() { success = GetRemoteFile(url, output, error, nullptr, "application/x-www-form-urlencoded", post_data.c_str(), std::vector<std::string>(), nullptr, 5); };
