...
try {
	...
	if (id.empty()) {
		auto func = [&]() { success = GetRemoteFile("https://mixer.com/api/v1/users/current", output, error, nullptr, "application/json", nullptr, headers, nullptr, 5); };

		ExecThreadedWithoutBlocking(func, QTStr("Auth.LoadingChannel.Title"), QTStr("Auth.LoadingChannel.Text").arg(service()));
		if (!success || output.empty())
			throw ErrorInfo("Failed to get user info from remote", error);
	...
	auto func = [&]() { success = GetRemoteFile(url.c_str(), output, error, nullptr, "application/json", nullptr, headers, nullptr, 5); };
...
bool MixerAuth::RetryLogin()
{
	...
	return GetToken(MIXER_TOKEN_URL, client_id, MIXER_SCOPE_VERSION, QT_TO_UTF8(login.GetCode()), true);
}
...
std::shared_ptr<Auth> MixerAuth::Login(QWidget *parent)
{
	...
	if (!auth->GetToken(MIXER_TOKEN_URL, client_id, MIXER_SCOPE_VERSION, QT_TO_UTF8(login.GetCode()))) {
		return nullptr;
	}
