...
PLSBasicPreview::PLSBasicPreview(QWidget *parent, Qt::WindowFlags flags) : PLSQTDisplay(parent, flags)
{
	ResetScrollingOffset();
	setMouseTracking(true);
}

PLSBasicPreview::~PLSBasicPreview()
{
	obs_enter_graphics();
...
vec2 PLSBasicPreview::GetMouseEventPos(QMouseEvent *event)
{
	...
	vec2 pos;
	vec2_set(&pos, (float(event->x()) - main->previewX / pixelRatio) * scale, (float(event->y()) - main->previewY / pixelRatio) * scale);
...
static inline vec2 GetPLSScreenSize()
{
	obs_video_info ovi;
	vec2 size;
	vec2_zero(&size);
...
vec3 PLSBasicPreview::GetSnapOffset(const vec3 &tl, const vec3 &br)
{
	PLSBasic *main = reinterpret_cast<PLSBasic *>(App()->GetMainWindow());
	vec2 screenSize = GetPLSScreenSize();
	vec3 clampOffset;
	...
	const float clampDist = config_get_double(GetGlobalConfig(), "BasicWindow", "SnapDistance") / main->previewScale;
...
bool PLSBasicPreview::SelectedAtPos(const vec2 &pos)
{
	PLSBasic *main = reinterpret_cast<PLSBasic *>(App()->GetMainWindow());
...
struct HandleFindData {
	...
	inline HandleFindData(const vec2 &pos_, float scale) : pos(pos_), radius(HANDLE_SEL_RADIUS / scale) { matrix4_identity(&parent_xform); }

	inline HandleFindData(const HandleFindData &hfd, obs_sceneitem_t *parent) : pos(hfd.pos), radius(hfd.radius), item(hfd.item), handle(hfd.handle)
	{
...
void PLSBasicPreview::GetStretchHandleData(const vec2 &pos)
{
	PLSBasic *main = reinterpret_cast<PLSBasic *>(App()->GetMainWindow());
...
void PLSBasicPreview::keyPressEvent(QKeyEvent *event)
{
	if (!IsFixedScaling() || event->isAutoRepeat()) {
		PLSQTDisplay::keyPressEvent(event);
		return;
	}
...
void PLSBasicPreview::keyReleaseEvent(QKeyEvent *event)
{
	if (event->isAutoRepeat()) {
		PLSQTDisplay::keyReleaseEvent(event);
		return;
	}
...
void PLSBasicPreview::wheelEvent(QWheelEvent *event)
{
	if (scrollMode && IsFixedScaling() && event->orientation() == Qt::Vertical) {
		if (event->delta() > 0)
	...
	PLSQTDisplay::wheelEvent(event);
}

void PLSBasicPreview::mousePressEvent(QMouseEvent *event)
{
...
void PLSBasicPreview::DoSelect(const vec2 &pos)
{
	PLSBasic *main = reinterpret_cast<PLSBasic *>(App()->GetMainWindow());
...
void PLSBasicPreview::DoCtrlSelect(const vec2 &pos)
{
...
void PLSBasicPreview::ProcessClick(const vec2 &pos)
{
	Qt::KeyboardModifiers modifiers = QGuiApplication::keyboardModifiers();
...
static bool GetSourceSnapOffset(obs_scene_t *scene, obs_sceneitem_t *item, void *param)
{
	...
	vec3 t[4] = {GetTransformedPos(0.0f, 0.0f, boxTransform), GetTransformedPos(1.0f, 0.0f, boxTransform), GetTransformedPos(0.0f, 1.0f, boxTransform),
...
void PLSBasicPreview::SnapItemMovement(vec2 &offset)
{
	PLSBasic *main = reinterpret_cast<PLSBasic *>(App()->GetMainWindow());
	OBSScene scene = main->GetCurrentScene();
...
void PLSBasicPreview::MoveItems(const vec2 &pos)
{
	Qt::KeyboardModifiers modifiers = QGuiApplication::keyboardModifiers();
	PLSBasic *main = reinterpret_cast<PLSBasic *>(App()->GetMainWindow());
	OBSScene scene = main->GetCurrentScene();
...
static bool IntersectBox(matrix4 transform, float x1, float x2, float y1, float y2)
{
	...
	if (IntersectLine(x1, x1, x3, x4, y1, y2, y3, y4) || IntersectLine(x1, x2, x3, x4, y1, y1, y3, y4) || IntersectLine(x2, x2, x3, x4, y1, y2, y3, y4) ||
...
bool PLSBasicPreview::FindSelected(obs_scene_t *scene, obs_sceneitem_t *item, void *param)
{
...
static bool FindItemsInBox(obs_scene_t *scene, obs_sceneitem_t *item, void *param)
{
	...
	if (transform.t.x + transform.x.x + transform.y.x > x1 && transform.t.x + transform.x.x + transform.y.x < x2 && transform.t.y + transform.x.y + transform.y.y > y1 &&
	...
	if (transform.t.x + 0.5 * (transform.x.x + transform.y.x) > x1 && transform.t.x + 0.5 * (transform.x.x + transform.y.x) < x2 && transform.t.y + 0.5 * (transform.x.y + transform.y.y) > y1 &&
...
void PLSBasicPreview::BoxItems(const vec2 &startPos, const vec2 &pos)
{
	PLSBasic *main = reinterpret_cast<PLSBasic *>(App()->GetMainWindow());
...
vec3 PLSBasicPreview::CalculateStretchPos(const vec3 &tl, const vec3 &br)
{
...
void PLSBasicPreview::ClampAspect(vec3 &tl, vec3 &br, vec2 &size, const vec2 &baseSize)
{
	...
	if (stretchHandle == ItemHandle::TopLeft || stretchHandle == ItemHandle::TopRight || stretchHandle == ItemHandle::BottomLeft || stretchHandle == ItemHandle::BottomRight) {
...
void PLSBasicPreview::SnapStretchingToScreen(vec3 &tl, vec3 &br)
{
	uint32_t stretchFlags = (uint32_t)stretchHandle;
...
void PLSBasicPreview::CropItem(const vec2 &pos)
{
...
void PLSBasicPreview::StretchItem(const vec2 &pos)
{
	Qt::KeyboardModifiers modifiers = QGuiApplication::keyboardModifiers();
	obs_bounds_type boundsType = obs_sceneitem_get_bounds_type(stretchItem);
	uint32_t stretchFlags = (uint32_t)stretchHandle;
...
void PLSBasicPreview::mouseMoveEvent(QMouseEvent *event)
{
...
void PLSBasicPreview::leaveEvent(QEvent *)
{
	std::lock_guard<std::mutex> lock(selectMutex);
	if (!selectionBox)
		hoveredPreviewItems.clear();
...
bool PLSBasicPreview::DrawSelectedOverflow(obs_scene_t *scene, obs_sceneitem_t *item, void *param)
{
	if (obs_sceneitem_locked(item))
		return true;
	...
	bool select = config_get_bool(GetGlobalConfig(), "BasicWindow", "OverflowSelectionHidden");
	...
	bool always = config_get_bool(GetGlobalConfig(), "BasicWindow", "OverflowAlwaysVisible");

	if (!always && !obs_sceneitem_selected(item))
		return true;

	PLSBasicPreview *prev = reinterpret_cast<PLSBasicPreview *>(param);
...
bool PLSBasicPreview::DrawSelectedItem(obs_scene_t *scene, obs_sceneitem_t *item, void *param)
{
	if (obs_sceneitem_locked(item))
		return true;
	...
	vec4_set(&red, 0.765f, 0.255f, 0.318f, 1.0f);
	vec4_set(&green, 0.0f, 1.0f, 0.0f, 1.0f);
	vec4_set(&blue, 0.0f, 0.569f, 1.0f, 1.0f);
	vec4_set(&yellow, 0.937f, 0.988f, 0.208f, 1.0f);
...
bool PLSBasicPreview::DrawSelectionBox(float x1, float y1, float x2, float y2, gs_vertbuffer_t *rectFill)
{
	x1 = std::round(x1);
	x2 = std::round(x2);
	y1 = std::round(y1);
...
void PLSBasicPreview::DrawOverflow()
{
	if (locked)
		return;
...
void PLSBasicPreview::DrawSceneEditing()
{
	if (locked)
		return;
	...
	if (selectionBox) {
		...
		DrawSelectionBox(startPos.x * main->previewScale, startPos.y * main->previewScale, mousePos.x * main->previewScale, mousePos.y * main->previewScale, rectFill);
	}
...
void PLSBasicPreview::ResetScrollingOffset()
{
	vec2_zero(&scrollingOffset);
}

void PLSBasicPreview::SetScalingLevel(int32_t newScalingLevelVal)
{
...
void PLSBasicPreview::SetScalingAmount(float newScalingAmountVal)
{
...
PLSBasicPreview *PLSBasicPreview::Get()
{
	return PLSBasic::Get()->ui->preview;
}
