...
void AutoConfigTestPage::TestBandwidthThread()
{
	...
	if (servers.size() < 3 || wiz->serviceName == "Mixer.com - FTL" || wiz->serviceName.substr(0, 11) == "Restream.io") {
		servers.resize(1);
	...
	OBSOutput output = obs_output_create(output_type, "test_stream", nullptr, nullptr);
	...
	for (size_t i = 0; i < servers.size(); i++) {
		...
		QMetaObject::invokeMethod(this, "UpdateMessage", Q_ARG(QString, QTStr(TEST_BW_CONNECTING).arg(server.name.c_str())));
		...
		QMetaObject::invokeMethod(this, "UpdateMessage", Q_ARG(QString, QTStr(TEST_BW_SERVER).arg(server.name.c_str())));
...
bool AutoConfigTestPage::TestSoftwareEncoding()
{
	...
	auto testRes = [&](long double div, int fps_num, int fps_den, bool force) {
		...
		QMetaObject::invokeMethod(this, "UpdateMessage", Q_ARG(QString, QTStr(TEST_RES_VAL).arg(cxStr, cyStr, fpsStr)));
...
void AutoConfigTestPage::FinalizeResults()
{
	...
	auto newLabel = [this](const char *str) -> QLabel * { return new QLabel(QTStr(str), this); };

	if (wiz->type != AutoConfig::Type::Recording) {
		const char *serverType = wiz->customServer ? "rtmp_custom" : "rtmp_common";

		OBSService service = obs_service_create(serverType, "temp_service", nullptr, nullptr);
		obs_service_release(service);
		...
		if (!wiz->customServer)
			form->addRow(newLabel("Basic.AutoConfig.StreamPage.Service"), new QLabel(wiz->serviceName.c_str(), ui->finishPage));
		form->addRow(newLabel("Basic.AutoConfig.StreamPage.Server"), new QLabel(wiz->serverName.c_str(), ui->finishPage));
		form->addRow(newLabel("Basic.Settings.Output.VideoBitrate"), new QLabel(QString::number(wiz->idealBitrate), ui->finishPage));
		form->addRow(newLabel(TEST_RESULT_SE), new QLabel(encName(wiz->streamingEncoder), ui->finishPage));
	}

	QString baseRes = QString("%1x%2").arg(QString::number(wiz->baseResolutionCX), QString::number(wiz->baseResolutionCY));
	QString scaleRes = QString("%1x%2").arg(QString::number(wiz->idealResolutionCX), QString::number(wiz->idealResolutionCY));

	if (wiz->recordingEncoder != AutoConfig::Encoder::Stream || wiz->recordingQuality != AutoConfig::Quality::Stream)
