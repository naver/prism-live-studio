...
#define MAX_READ_SAMPLES 48000 // per channel
...
void sound_touch_wrapper::setup_soundtouch(const st_audio_info &stream, const st_effect_info &effect)
{
	//------------------ clear old state --------------------
	clear();
	...
	sound_touch.setTempoChange(effect.tempo);
	sound_touch.setPitchSemiTones((int)effect.pitch);
	sound_touch.setRateChange(effect.rate);
	...
	float *rb = new float[sizeof(float) * MAX_READ_SAMPLES * stream.channels];
	read_buffer = std::shared_ptr<float>(rb, std::default_delete<float[]>());
}

void sound_touch_wrapper::get_setup_info(st_audio_info &stream, st_effect_info &effect)
{
...
void sound_touch_wrapper::pop_audio()
{
	...
	if (saved_frames >= stream_settings.sample_rate) {
		PLS_PLUGIN_WARN("[sound-touch filter] Buffer of audio is too large. saved_frames_per_chn:%lld samplerate:%u", saved_frames, stream_settings.sample_rate);
	}
...
void sound_touch_wrapper::push_result()
{
	...
	if (output_frames > 0) {
		...
		if (push_buffer_size < push_size) {
			...
			float *sb = new float[push_size];
			push_buffer = std::shared_ptr<float>(sb, std::default_delete<float[]>());
		}
...
void sound_touch_wrapper::clear()
{
	...
	saved_frames = 0;
	circlebuf_pop_front(&save_buffer, NULL, save_buffer.size);
	info_queue.clear();
}
