...
void pls_logex(bool kr, pls_log_level_t log_level, const char *file_name, int file_line, const char *fields[][2], int field_count, int arg_count, const char *format, ...)
{
	va_list args;
	va_start(args, format);
	pls_logvaex(kr, log_level, obs_module_name(), file_name, file_line, fields, field_count, arg_count, format, args);
	va_end(args);
}
...
{
	pls_action_log(kr, obs_module_name(), controls, action, file_name, file_line);
}
