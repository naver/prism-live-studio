...
echo obs configure.
echo ******************************************************************************
echo cmake -Wno-dev -S "%OBS_SRC_DIR%" -B "%OBS_BUILD_ARCH_DIR%" -G "%GENERATOR%" -A %ARCH% -T v142 -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DCMAKE_PREFIX_PATH="%QTDIR%" -DRELEASE_CANDIDATE=%RELEASE_CANDIDATE% -DCOPIED_DEPENDENCIES=false -DCOPY_DEPENDENCIES=true -DBUILD_CAPTIONS=true -DCOMPILE_D3D12_HOOK=true -DBUILD_BROWSER=true -DCEF_ROOT_DIR=%CEF_ROOT_DIR%
echo ******************************************************************************
cmake -Wno-dev -S "%OBS_SRC_DIR%" -B "%OBS_BUILD_ARCH_DIR%" -G "%GENERATOR%" -A %ARCH% -T v142 -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DCMAKE_PREFIX_PATH="%QTDIR%" -DRELEASE_CANDIDATE=%RELEASE_CANDIDATE% -DCOPIED_DEPENDENCIES=false -DCOPY_DEPENDENCIES=true -DBUILD_CAPTIONS=true -DCOMPILE_D3D12_HOOK=true -DBUILD_BROWSER=true -DCEF_ROOT_DIR=%CEF_ROOT_DIR%
