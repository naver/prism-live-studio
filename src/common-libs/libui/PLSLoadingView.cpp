...
{
	pls_uistep_v2_set_custom_show_hide_name(this, QByteArrayLiteral("PLSLoadingView"));
...
PLSLoadingView *PLSLoadingView::newLoadingView(QWidget *parent, int absoluteTop, const QString &pathImage, const QSize &defaultSize)
{
	PLSLoadingView *loadingView = pls_new<PLSLoadingView>(parent, pathImage);
	loadingView->m_absoluteTop = absoluteTop;
	loadingView->m_defaultSize = defaultSize;
	loadingView->raise();
	loadingView->setAutoResizeFrom(parent);
	loadingView->show();
	return loadingView;
...
{
	auto minWidth = static_cast<int>(m_dpi * m_defaultSize.width());
	auto minHeight = static_cast<int>(m_dpi * (m_defaultSize.height() + m_absoluteTop + 1));
...
void PLSLoadingView::paintEvent(QPaintEvent *)
{
	...
	QRectF svgRect(0, 0, m_defaultSize.width() * m_dpi, m_defaultSize.height() * m_dpi);
