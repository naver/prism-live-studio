...
public:
	...
	bool validateCurrentPage() override { return wizardView->validateCurrentPage(); }
	int nextId() const override { return wizardView->nextId(); }

	void done(int result) override { wizardView->done(result); }

	PLSWizardView *getWizardView() override { return wizardView; }
...
{
	impl = pls_new<PLSWizardImpl>(this, this->content());
